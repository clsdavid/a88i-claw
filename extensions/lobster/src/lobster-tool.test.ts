...
import type { AutoCrabPluginApi, AutoCrabPluginToolContext } from "autocrab/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<AutoCrabPluginApi> = {}): AutoCrabPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<AutoCrabPluginToolContext> = {}): AutoCrabPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: AutoCrabPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
