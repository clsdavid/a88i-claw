...
`autocrab.invoke --each`) without adding new AutoCrab code per workflow.
...
This extension depends on AutoCrab internal modules (the embedded agent runner).
It is intended to ship as a **bundled** AutoCrab extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.autocrab/extensions` as a standalone plugin directory.
