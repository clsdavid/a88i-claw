...
import Ajv from "ajv";
import { resolvePreferredAutoCrabTmpDir } from "autocrab/plugin-sdk/llm-task";
// NOTE: This extension is intended to be bundled with AutoCrab.
// When running from source (tests/dev), AutoCrab internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { AutoCrabPluginApi } from "autocrab/plugin-sdk/llm-task";
...
export function createLlmTaskTool(api: AutoCrabPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via autocrab.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredAutoCrabTmpDir(), "autocrab-llm-task-"),
        );
