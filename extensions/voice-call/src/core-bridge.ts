...
function resolveAutoCrabRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.AUTOCRAB_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set AUTOCRAB_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveAutoCrabRoot(), "dist", "extensionAPI.js");
