...
import type { PluginRuntime } from "autocrab/plugin-sdk/nostr";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.AUTOCRAB_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-nostr-"));
  process.env.AUTOCRAB_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.AUTOCRAB_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".autocrab");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.AUTOCRAB_STATE_DIR = previous;
    }
