  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/autocrab",
      },
    },
    session: { store: "/tmp/autocrab-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bautocrab\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "autocrab: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "AutoCrab" },
          },
        },
        eventPatch: {
