...
describe("thread binding lifecycle", () => {
  ...
  it("persists touched activity timestamps across restart when persistence is enabled", async () => {
    vi.useFakeTimers();
    const previousStateDir = process.env.AUTOCRAB_STATE_DIR;
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "autocrab-thread-bindings-"));
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.AUTOCRAB_STATE_DIR = previousStateDir;
      }
