...
describe("agent components", () => {
  const createCfg = (): AutoCrabConfig => ({}) as AutoCrabConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): AutoCrabConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/autocrab-sessions-test.json");
  });
