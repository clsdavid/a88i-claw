...
Read-only diff viewer plugin for **AutoCrab** agents.
...
Set plugin-wide defaults in `~/.autocrab/autocrab.json`:
...
Before:
AutoCrab supports plugins.

After:
AutoCrab supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/autocrab-diffs`).
