...
import type { AutoCrabPluginApi } from "autocrab/plugin-sdk/diffs";
...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-diffs-tool-"));
...
function createApi(): AutoCrabPluginApi {
  return {
    id: "diffs",
    name: "Diffs",
    description: "Diffs",
    ...
    runtime: {} as AutoCrabPluginApi["runtime"],
    logger: {
      info() {},
      warn() {},
      error() {},
