import type { RuntimeEnv, ReplyPayload, AutoCrabConfig } from "autocrab/plugin-sdk/tlon";
import { createLoggerBackedRuntime, createReplyPrefixOptions } from "autocrab/plugin-sdk/tlon";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.loadConfig() as AutoCrabConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in AutoCrab config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your AutoCrab config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.autocrab.ai/concepts/session#secure-dm-mode`;
