/**
 * Twitch channel plugin for AutoCrab.
 *
...
import { buildChannelConfigSchema } from "autocrab/plugin-sdk/twitch";
 ...
 * for AutoCrab. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: AutoCrabConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: AutoCrabConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: AutoCrabConfig): boolean => {
