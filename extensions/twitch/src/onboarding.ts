...
import {
  ...
  type WizardPrompter,
} from "autocrab/plugin-sdk/twitch";
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: AUTOCRAB_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: AutoCrabConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var AUTOCRAB_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.AUTOCRAB_TWITCH_ACCESS_TOKEN?.trim();
