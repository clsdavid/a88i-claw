...
import type { AutoCrabPluginApi } from "autocrab/plugin-sdk/device-pair";
import {
...
} from "autocrab/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: AutoCrabPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.AUTOCRAB_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: AutoCrabPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: AutoCrabPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the AutoCrab iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the AutoCrab iOS app:",
            "",
            "```",
            qrAscii,
            "```",
