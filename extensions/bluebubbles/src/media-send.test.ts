...
import type { AutoCrabConfig, PluginRuntime } from "autocrab/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): AutoCrabConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
