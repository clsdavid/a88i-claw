...
// Prefix for AutoCrab-generated action IDs to scope our handler
const AUTOCRAB_ACTION_PREFIX = "autocrab:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${AUTOCRAB_ACTION_PREFIX}`);

  // Handle AutoCrab modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
