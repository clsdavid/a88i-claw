...
metadata: { "autocrab": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == AutoCrab `sessions_spawn`
- **File I/O** == AutoCrab `read`/`write`
- **Remote fetch** == AutoCrab `web_fetch` (or `exec` with curl when POST is required)
