...
vi.mock("autocrab/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("autocrab/plugin-sdk/diagnostics-otel")>(
    "autocrab/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { AutoCrabPluginServiceContext } from "autocrab/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "autocrab/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/autocrab-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): AutoCrabPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("autocrab.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("autocrab.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("autocrab.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("autocrab.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("autocrab.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("autocrab.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("autocrab.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("autocrab.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("autocrab.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("autocrab.webhook.processed");
    expect(spanNames).toContain("autocrab.message.processed");
    expect(spanNames).toContain("autocrab.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["autocrab.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("autocrab.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "autocrab.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["autocrab.reason"]).toBe("string");
    expect(String(attrs?.["autocrab.reason"])).not.toContain(
