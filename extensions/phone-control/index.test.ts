...
} from "autocrab/plugin-sdk/phone-control";
...
describe("phone-control plugin", () => {
  it("arms sms.send as part of the writes group", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-phone-control-test-"));
    try {
      ...
      let command: AutoCrabPluginCommandDefinition | undefined;
