...
import type { AutoCrabPluginApi, AutoCrabPluginService } from "autocrab/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: AutoCrabPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AutoCrabPluginApi["config"]): string[] {
...
export default function register(api: AutoCrabPluginApi) {
  ...
  const timerService: AutoCrabPluginService = {
