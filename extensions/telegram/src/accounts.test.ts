...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", AUTOCRAB_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
      const cfg: AutoCrabConfig = {
        channels: {
          telegram: { accounts: { work: { botToken: "tok-work" } } },
        },
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): AutoCrabConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): AutoCrabConfig => ({
    channels: {
      telegram: {
