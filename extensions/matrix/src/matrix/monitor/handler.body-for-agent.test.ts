...
import type { PluginRuntime, RuntimeEnv, RuntimeLogger } from "autocrab/plugin-sdk/matrix";
  ...
  it("stores sender-labeled BodyForAgent for group thread messages", async () => {
    ...
    const core = {
      channel: {
        ...
        session: {
          resolveStorePath: vi.fn().mockReturnValue("/tmp/autocrab-test-session.json"),
