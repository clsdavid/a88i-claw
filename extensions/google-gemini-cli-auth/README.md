# Google Gemini CLI Auth (AutoCrab plugin)
...
```bash
autocrab plugins enable google-gemini-cli-auth
```
...
```bash
autocrab models auth login --provider google-gemini-cli --set-default
```
...
- `AUTOCRAB_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `AUTOCRAB_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
