...
read_when:
  - You want to place an outbound voice call from AutoCrab
...
Voice calls for AutoCrab via a plugin. Supports outbound notifications and
...
- Use `autocrab voicecall ...` or the `voice_call` tool
...
```bash
autocrab plugins install ./extensions/voice-call
