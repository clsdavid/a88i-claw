...
read_when:
  - You want AutoCrab to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables AutoCrab to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`autocrab onboard`) and `autocrab channels add` list optional channel plugins.
...
```bash
autocrab plugins install @autocrab/nostr
```

Use a local checkout (dev workflows):

```bash
autocrab plugins install --link <path-to-autocrab>/extensions/nostr
```
...
```json
{
  "channels": {
    "nostr": {
      ...
      "profile": {
        "name": "autocrab",
        "displayName": "AutoCrab",
        "about": "Personal assistant DM bot",
        "picture": "https://example.com/avatar.png",
        "banner": "https://example.com/banner.png",
        "website": "https://example.com",
        "nip05": "autocrab@example.com",
        "lud16": "autocrab@example.com"
      }
    }
  }
}
