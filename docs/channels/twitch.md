...
read_when:
  - Setting up Twitch chat integration for AutoCrab
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. AutoCrab connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
autocrab plugins install @autocrab/twitch
```

Local checkout (when running from a git repo):

```bash
autocrab plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `AUTOCRAB_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "autocrab", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use AUTOCRAB_TWITCH_ACCESS_TOKEN env var)
...
```bash
AUTOCRAB_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
