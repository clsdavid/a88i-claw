...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects AutoCrab to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
autocrab plugins install @autocrab/feishu
```

Local checkout (when running from a git repo):

```bash
autocrab plugins install ./extensions/feishu
```
...
If you just installed AutoCrab, run the wizard:
...
2. Configuring app credentials in AutoCrab
...
1. You already ran `autocrab channels add` for Feishu
2. The gateway is running (`autocrab gateway status`)
...
```bash
autocrab pairing approve feishu <CODE>
```
...
2. Run `autocrab logs --follow` and look for `chat_id`
...
2. Run `autocrab logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `autocrab gateway status`  | Show gateway status           |
| `autocrab gateway install` | Install/start gateway service |
| `autocrab gateway stop`    | Stop gateway service          |
| `autocrab gateway restart` | Restart gateway service       |
| `autocrab logs --follow`   | Tail gateway logs             |
...
4. Check logs: `autocrab logs --follow`
...
5. Ensure the gateway is running: `autocrab gateway status`
6. Check logs: `autocrab logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.autocrab/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.autocrab/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
