...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure AutoCrab HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `autocrab pairing approve slack <code>`.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while AutoCrab is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
AutoCrab supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, AutoCrab falls back to normal delivery for remaining payloads.
