...
The onboarding wizard is the **recommended** way to set up AutoCrab on macOS,
...
`autocrab dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
(`web_fetch` works without a key). Easiest path: `autocrab configure --section web`
...
2. **Workspace** — Location for agent files (default `~/.autocrab/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `autocrab doctor` first.
</Note>
...
Use `autocrab agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.autocrab/workspace-<agentId>`.
...
- CLI command reference: [`autocrab onboard`](/cli/onboard)
