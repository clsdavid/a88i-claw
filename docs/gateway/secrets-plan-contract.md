...
read_when:
  - Generating or reviewing `autocrab secrets apply` plans
...
This page defines the strict contract enforced by `autocrab secrets apply`.

If a target does not match these rules, apply fails before mutating configuration.

## Plan file shape

`autocrab secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
- `secrets apply` writes supported `autocrab.json` targets, supported `auth-profiles.json` targets, and optional scrub targets.
...
If apply fails with an invalid target path message, regenerate the plan with `autocrab secrets configure` or fix the target path to a supported shape above.
