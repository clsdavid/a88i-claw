...
description: "Complete field-by-field reference for ~/.autocrab/autocrab.json"
summary: "Complete reference for every AutoCrab config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.autocrab/autocrab.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — AutoCrab uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.autocrab/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `autocrab doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `autocrab doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["autocrab-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-autocrab",
          requireMention: false,
...
- AutoCrab additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `autocrab plugins install @autocrab/mattermost`.
...
- `commands.callbackUrl` must resolve to the AutoCrab gateway endpoint and be reachable from the Mattermost server.
...
AutoCrab spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `autocrab channels add` (or channel onboarding) while still on a single-account top-level channel config, AutoCrab moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `autocrab doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@autocrab", "autocrab"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@autocrab"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `autocrab.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, AutoCrab auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/autocrab" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, AutoCrab assumes `anthropic` (deprecated).
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.autocrab/sandboxes",
        docker: {
          ...
          apparmorProfile: "autocrab-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "autocrab-sandbox-browser:bookworm-slim",
          network: "autocrab-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.autocrab/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `autocrab.json`.
noVNC observer access uses VNC auth by default and AutoCrab emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `autocrab-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from AUTOCRAB_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `AUTOCRAB_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `AUTOCRAB_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `AUTOCRAB_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        agentDir: "~/.autocrab/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@autocrab"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.autocrab/workspace-home" },
      { id: "work", workspace: "~/.autocrab/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, AutoCrab resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.autocrab/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.autocrab/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.autocrab/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.autocrab/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, AutoCrab starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.autocrab/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, AutoCrab treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:autocrab`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.autocrab/attachments/<uuid>/` with a `.manifest.json`.
...
AutoCrab uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.autocrab/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `AUTOCRAB_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), AutoCrab forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `autocrab onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `autocrab onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `autocrab onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `autocrab onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `autocrab onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `autocrab onboard --auth-choice minimax-api`.
...
- Loaded from `~/.autocrab/extensions`, `<workspace>/.autocrab/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `autocrab plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      autocrab: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or AUTOCRAB_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `AUTOCRAB_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `AUTOCRAB_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
```bash
AUTOCRAB_CONFIG_PATH=~/.autocrab/a.json \
AUTOCRAB_STATE_DIR=~/.autocrab-a \
autocrab gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.autocrab-dev` + port `19001`), `--profile <name>` (uses `~/.autocrab-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.autocrab/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-autocrab-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `AUTOCRAB_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.autocrab/workspace/canvas",
    liveReload: true,
    // enabled: false, // or AUTOCRAB_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__autocrab__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__autocrab__/a2ui/`
...
- Also serves A2UI at `/__autocrab__/a2ui/`.
...
- Hostname defaults to `autocrab`. Override with `AUTOCRAB_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.autocrab/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `autocrab dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.autocrab/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${AUTOCRAB_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `autocrab.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.autocrab/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/autocrab-vault-resolver",
...
- Legacy OAuth imports from `~/.autocrab/credentials/oauth.json`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/autocrab/autocrab.log",
    consoleLevel: "info",
...
- Default log file: `/tmp/autocrab/autocrab-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one AutoCrab.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `AUTOCRAB_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `autocrab doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `autocrab.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
