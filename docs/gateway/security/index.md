...
> AutoCrab is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
AutoCrab security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `autocrab security audit`
...
```bash
autocrab security audit
autocrab security audit --deep
autocrab security audit --fix
autocrab security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

AutoCrab is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
AutoCrab assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.autocrab`, including `autocrab.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- AutoCrab can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want AutoCrab, use one VPS/host per user.
...
AutoCrab is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/clsdavid/autocrab/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, AutoCrab also attempts a best-effort live Gateway probe.
...
| -------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full AutoCrab state                                 | filesystem perms on `~/.autocrab`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.autocrab/autocrab.json`                                                   | yes      |
...
`autocrab security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`autocrab security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in AutoCrab config
schema:
...
gateway:
  ...
  auth:
    mode: password
    password: ${AUTOCRAB_GATEWAY_PASSWORD}
```
...
- AutoCrab gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from AutoCrab responses.
...
AutoCrab stores session transcripts on disk under `~/.autocrab/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.autocrab` (see the audit section below). If you need
...
AutoCrab can refresh the skills list mid-session:
...
- If you install plugins from npm (`autocrab plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.autocrab/extensions/<pluginId>/` (or `$AUTOCRAB_STATE_DIR/extensions/<pluginId>/`).
  - AutoCrab uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
autocrab pairing list <channel>
autocrab pairing approve <channel> <code>
```
...
By default, AutoCrab routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
AutoCrab has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.autocrab/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.autocrab or your logs.”

## Unsafe external content bypass flags

AutoCrab includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.autocrab/autocrab.json`: `600` (user read/write only)
- `~/.autocrab`: `700` (user only)

`autocrab doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `AUTOCRAB_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__autocrab__/canvas/` and `/__autocrab__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
If you run AutoCrab with Docker on a VPS, remember that published container ports
...
The Gateway broadcasts its presence via mDNS (`_autocrab-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `AUTOCRAB_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `autocrab doctor --generate-gateway-token`.
...
paths, set `AUTOCRAB_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `AUTOCRAB_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `AUTOCRAB_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), AutoCrab
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. AutoCrab verifies the identity by resolving the
...
- AutoCrab will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.autocrab/` (or `$AUTOCRAB_STATE_DIR/`) may contain secrets or private data:

- `autocrab.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `autocrab status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@autocrab", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.autocrab`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.autocrab/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `autocrab` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only AutoCrab clients can connect.
- Disable browser proxy routing when you don’t need it (`gateway.nodes.browser.mode="off"`).
- Chrome extension relay mode is **not** “safer”; it can take over your existing Chrome tabs. Assume it can act as you in whatever that tab/profile can reach.

### Browser SSRF policy (trusted-network default)

AutoCrab’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default AutoCrab limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `autocrab gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `AUTOCRAB_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/autocrab/autocrab-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.autocrab/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `autocrab security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + AutoCrab version
...
Found a vulnerability in AutoCrab? Please report responsibly:

1. Email: [security@autocrab.ai](mailto:security@autocrab.ai)
