...
- During heartbeat runs, AutoCrab treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `autocrab sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, AutoCrab skips the heartbeat run entirely (no model call).
...
```bash
autocrab system event --text "Check for urgent follow-ups" --mode now
```
