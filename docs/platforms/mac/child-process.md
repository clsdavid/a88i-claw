...
service via the external `autocrab` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.autocrab.gateway`
  (or `ai.autocrab.<profile>` when using `--profile`/`AUTOCRAB_PROFILE`; legacy `com.autocrab.*` is supported).
