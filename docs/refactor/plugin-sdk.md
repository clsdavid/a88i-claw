...
- Publish as `autocrab/plugin-sdk` (or export from core under `autocrab/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `AutoCrabPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AutoCrabConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AutoCrabConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AutoCrabConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AutoCrabConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AutoCrabConfig): string;
  };
};
```
...
- Introduce `autocrab/plugin-sdk`.
- Add `api.runtime` to `AutoCrabPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `autocrabRuntime: ">=2026.2.0"`).
