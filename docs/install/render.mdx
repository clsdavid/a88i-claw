...
Deploy AutoCrab on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/autocrab/autocrab)
...
repository configures everything needed to run AutoCrab:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: AUTOCRAB_STATE_DIR
        value: /data/.autocrab
      - key: AUTOCRAB_WORKSPACE_DIR
        value: /data/workspace
      - key: AUTOCRAB_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/autocrab`.
...
If you use the original AutoCrab repository, Render will not auto-deploy your AutoCrab. To update it, run a manual Blueprint sync from the dashboard.
...
For AutoCrab, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any AutoCrab host.
