...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let AutoCrab run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask AutoCrab in plain language to "run this in Codex" or "start Claude Code in a thread", AutoCrab should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want AutoCrab-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | AutoCrab native sub-agent runtime  |
...
- AutoCrab binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, AutoCrab returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning AutoCrab agent id.
...
- AutoCrab ensures the configured ACP session exists before use.
...
- If `agentId` is omitted, AutoCrab uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, AutoCrab may default to persistent behavior per runtime path
...
ACP sessions currently run on the host runtime, not inside the AutoCrab sandbox.
...
If no target resolves, AutoCrab returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, AutoCrab returns a clear unsupported-control error.
...
When AutoCrab uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal AutoCrab `agentId` path).
...
```bash
autocrab plugins install acpx
autocrab config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
autocrab plugins install ./extensions/acpx
```
...
By default, the acpx plugin (published as `@autocrab/acpx`) uses the plugin-local pinned binary:
...
- Relative paths resolve from AutoCrab workspace directory.
...
- AutoCrab startup remains non-blocking while the backend health check runs.
...
```bash
autocrab config set plugins.entries.acpx.config.permissionMode approve-all
autocrab config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** AutoCrab currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
