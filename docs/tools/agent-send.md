---
summary: "Direct `autocrab agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `autocrab agent` (direct agent runs)

`autocrab agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `autocrab message --target`).
...
```bash
autocrab agent --to +15555550123 --message "status update"
...
autocrab agent --to +15555550123 --message "Summon reply" --deliver
