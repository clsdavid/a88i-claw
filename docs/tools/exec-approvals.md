...
- **gateway host** → `autocrab` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.autocrab/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.autocrab/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `autocrab approvals allowlist ...`).
- `autocrab security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `autocrab doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.autocrab/exec-approvals.json` directly.

CLI: `autocrab approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
