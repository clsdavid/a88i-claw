---
summary: "CLI reference for `autocrab configure` (interactive configuration prompts)"
read_when:
...
Tip: `autocrab config` without a subcommand opens the same wizard. Use
`autocrab config get|set|unset` for non-interactive edits.
...
```bash
autocrab configure
autocrab configure --section model --section channels
```
