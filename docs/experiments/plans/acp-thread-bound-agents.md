...
This plan defines how AutoCrab should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- AutoCrab core owns ACP control plane concerns
...
AutoCrab should not reimplement ACP transport internals in core.
AutoCrab should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in AutoCrab, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under AutoCrab state dir
...
- ACP event stream is normalized to AutoCrab reply chunks
...
For the first production backend (`extensions/acpx`), AutoCrab and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses AutoCrab `sessionKey`)
...
- AutoCrab consumes ndjson events from `acpx --format json --json-strict`
