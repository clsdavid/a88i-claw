...
read_when:
  - Designing workspace memory (~/.autocrab/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep AutoCrab integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.autocrab/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.autocrab/workspace` as canonical human-readable memory.
...
```
~/.autocrab/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in AutoCrab**, but keep a separable core library.

### Why integrate into AutoCrab?

- AutoCrab already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `autocrab memory recall "…" --k 25 --since 30d`
  - `autocrab memory reflect --since 7d`
...
Pragmatic take for `~/.autocrab/workspace`:
