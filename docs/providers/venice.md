---
summary: "Use Venice AI privacy-focused models in AutoCrab"
read_when:
  - You want privacy-focused inference in AutoCrab
...
```bash
autocrab onboard --auth-choice venice-api-key
```
...
```bash
autocrab agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, AutoCrab shows all available Venice models. Pick based on your needs:
...
```bash
autocrab models set venice/claude-opus-45
autocrab models set venice/llama-3.3-70b
```

List all available models:

```bash
autocrab models list | grep venice
```

## Configure via `autocrab configure`

1. Run `autocrab configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
AutoCrab automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
autocrab agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
autocrab agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
autocrab agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
autocrab agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
autocrab agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `autocrab models list` to see currently available models. Some models may be temporarily offline.
