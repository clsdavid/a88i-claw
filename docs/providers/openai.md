---
summary: "Use OpenAI via API keys or Codex subscription in AutoCrab"
read_when:
  - You want to use OpenAI models in AutoCrab
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like AutoCrab.
...
```bash
autocrab onboard --auth-choice openai-api-key
# or non-interactive
autocrab onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. AutoCrab forwards both through the `openai/*` Responses path.
...
autocrab onboard --auth-choice openai-codex

# Or run OAuth directly
autocrab models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. AutoCrab
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

AutoCrab uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), AutoCrab also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. AutoCrab enables it by default for
...
AutoCrab, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), AutoCrab now auto-enables OpenAI server-side
