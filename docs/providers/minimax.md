---
summary: "Use MiniMax M2.5 in AutoCrab"
read_when:
  - You want MiniMax models in AutoCrab
...
- **Compatibility:** AutoCrab still accepts legacy `MiniMax-M2.5-Lightning` configs, but prefer
...
```bash
autocrab plugins enable minimax-portal-auth  # skip if already loaded.
autocrab gateway restart  # restart if gateway is already running
autocrab onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/autocrab/autocrab/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `autocrab.json`:
...
- Use `autocrab models list` and `autocrab models set minimax/MiniMax-M2.5` to switch.
...
- Running `autocrab configure` and selecting **MiniMax M2.5**, or
