            ...
            if msg_type == "req":
                ...
                if method == "connect":
                    await websocket.send_json({
                         ...
                         "payload": {
                             "features": {
                                 "methods": ["health", "channels.status", "doctor.memory.status", "sessions.list", "memory.search", "chat.history", "chat.send", "node.list", "agent.identity.get", "agents.list", "tools.catalog", "device.pair.list", "models.list", "config.get", "config.schema", "config.schema.lookup", "cron.status", "cron.list", "cron.runs"]
                             }
                         }
                    })
                elif method == "ping":
                    ...
                    query = msg.get("params", {}).get("query", "").lower()
                    results = []
                ...
                elif method == "config.schema.lookup":
