---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for AutoCrab deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, AutoCrab cron scheduling for periodic checks, or version status checks on a machine running AutoCrab (laptop, workstation, Pi, VPS).
---

# AutoCrab Host Hardening

## Overview

Assess and harden the host running AutoCrab, then align it to a user-defined risk tolerance without breaking access. Use AutoCrab security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim AutoCrab changes the host firewall, SSH, or OS updates; it does not.
...
5. AutoCrab gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if AutoCrab can access sensitive data.
...
### 2) Run AutoCrab security audits (read-only)

As part of the default read-only checks, run `autocrab security audit --deep`. Only offer alternatives if the user requests them:

1. `autocrab security audit` (faster, non-probing)
2. `autocrab security audit --json` (structured output)

Offer to apply AutoCrab safe defaults (numbered):

1. `autocrab security audit --fix`

Be explicit that `--fix` only tightens AutoCrab defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check AutoCrab version/update status (read-only)

As part of the default read-only checks, run `autocrab update status`.
...
- Credential hygiene notes (location of AutoCrab creds, prefer disk encryption)
...
- AutoCrab security audit (re-run)
...
After AutoCrab install or first hardening pass, run at least one baseline audit and version check:

- `autocrab security audit`
- `autocrab security audit --deep`
- `autocrab update status`

Ongoing monitoring is recommended. Use the AutoCrab cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `autocrab cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `autocrab update status`
...
Before creating, `autocrab cron list` and match on exact `name`. If found, `autocrab cron edit <id> ...`.
If not found, `autocrab cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `autocrab update status` (preferred for source checkouts and channels)
2. `npm view autocrab version` (published npm version)

## AutoCrab command accuracy

Use only supported commands and flags:

- `autocrab security audit [--deep] [--fix] [--json]`
- `autocrab status` / `autocrab status --deep`
- `autocrab health --json`
- `autocrab update status`
- `autocrab cron add|list|runs|run`

Do not invent CLI flags or imply AutoCrab enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by AutoCrab compaction:
