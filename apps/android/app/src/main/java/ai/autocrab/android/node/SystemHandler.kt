...
private const val NOTIFICATION_CHANNEL_BASE_ID = "autocrab.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "AutoCrab Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "AutoCrab Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "AutoCrab Active")
      }
