...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: AutoCrabProtocol.AnyCodable(["ok": AutoCrabProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: AutoCrabProtocol.AnyCodable(["ok": AutoCrabProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = AutoCrabProtocol.AnyCodable([
            "runId": AutoCrabProtocol.AnyCodable("run-1"),
            "sessionKey": AutoCrabProtocol.AnyCodable("main"),
            "state": AutoCrabProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: AutoCrabProtocol.AnyCodable(["a": AutoCrabProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
