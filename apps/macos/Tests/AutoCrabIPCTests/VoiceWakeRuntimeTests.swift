...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["autocrab"]
        let text = "hey autocrab"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey autocrab do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["autocrab"], minPostTriggerGap: 0.3)
