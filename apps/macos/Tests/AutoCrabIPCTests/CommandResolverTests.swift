...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersAutoCrabBinary() throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()
        CommandResolver.setProjectRoot(tmp.path)

        let autocrabPath = tmp.appendingPathComponent("node_modules/.bin/autocrab")
        try makeExecutableForTests(at: autocrabPath)

        let cmd = CommandResolver.autocrabCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([autocrabPath.path, "gateway"]))
    }
    ...
    @Test func prefersAutoCrabBinaryOverPnpm() throws {
        let defaults = self.makeLocalDefaults()
        ...
        let autocrabPath = binDir.appendingPathComponent("autocrab")
        ...
        #expect(cmd.prefix(2).elementsEqual([autocrabPath.path, "rpc"]))
    }

    @Test func usesAutoCrabBinaryWithoutNodeRuntime() throws {
        let defaults = self.makeLocalDefaults()
    ...
    @Test func fallsBackToPnpm() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "autocrab", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "autocrab", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() {
        ...
        defaults.set("autocrab@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/autocrab", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "autocrab@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/autocrab'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("autocrab"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() throws {
        ...
        #expect(cmd.first == autocrabPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
