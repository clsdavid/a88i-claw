...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.autocrab", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in AutoCrabBonjour.gatewayServiceDomains {
            let browser = GatewayDiscoveryBrowserSupport.makeBrowser(
                serviceType: AutoCrabBonjour.gatewayServiceType,
                domain: domain,
                queueLabelPrefix: "ai.autocrab.macos.gateway-discovery",
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = AutoCrabBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = AutoCrabBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = AutoCrabBonjour.wideAreaGatewayServiceDomain else { return false }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (AutoCrab)", with: "")
