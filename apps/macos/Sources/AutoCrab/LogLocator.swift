...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["AUTOCRAB_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/autocrab")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("autocrab-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("autocrab-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/autocrab/ (rolling or stdout), or nil if none exist.
