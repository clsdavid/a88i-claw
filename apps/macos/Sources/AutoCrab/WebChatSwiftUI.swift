...
private let webChatSwiftLogger = Logger(subsystem: "ai.autocrab", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: AutoCrabChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AutoCrabChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(AutoCrabChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AutoCrabChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AutoCrabGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    AutoCrabAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AutoCrabChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AutoCrabChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AutoCrabChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: AutoCrabChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AutoCrabChatView>) -> NSViewController
    {
