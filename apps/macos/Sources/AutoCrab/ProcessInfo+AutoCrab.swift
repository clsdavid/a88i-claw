...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.autocrab.mac`) even if the shipped
    ...
    {
        if environment["AUTOCRAB_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "autocrab.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "autocrab.nixMode") { return true }
