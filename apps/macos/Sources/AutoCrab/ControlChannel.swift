...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AutoCrabProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.autocrab", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AutoCrabKit.AnyCodable]()) {
                $0[$1.key] = AutoCrabKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AutoCrabProtocol.AnyCodable?) -> [String: AutoCrabProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AutoCrabProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AutoCrabKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AutoCrabProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("autocrab.control.heartbeat")
    static let controlAgentEvent = Notification.Name("autocrab.control.agent")
}
