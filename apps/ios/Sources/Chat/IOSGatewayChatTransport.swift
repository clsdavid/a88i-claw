...
struct IOSGatewayChatTransport: AutoCrabChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.autocrab", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> AutoCrabChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AutoCrabChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AutoCrabChatHistoryPayload {
        ...
        return try JSONDecoder().decode(AutoCrabChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [AutoCrabChatAttachmentPayload]) async throws -> AutoCrabChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AutoCrabChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(AutoCrabChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(AutoCrabGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AutoCrabGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AutoCrabChatEventPayload.self)
                        {
