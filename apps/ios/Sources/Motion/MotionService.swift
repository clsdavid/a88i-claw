...
final class MotionService: MotionServicing {
    func activities(params: AutoCrabMotionActivityParams) async throws -> AutoCrabMotionActivityPayload {
        ...
        let mapped: [AutoCrabMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return AutoCrabMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: AutoCrabPedometerParams) async throws -> AutoCrabPedometerPayload {
        ...
        let payload: AutoCrabPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = AutoCrabPedometerPayload(
