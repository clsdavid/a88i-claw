...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AutoCrabContactsSearchParams) async throws -> AutoCrabContactsSearchPayload {
        ...
        return AutoCrabContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AutoCrabContactsAddParams) async throws -> AutoCrabContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AutoCrabContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AutoCrabContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AutoCrabContactPayload {
        AutoCrabContactPayload(
