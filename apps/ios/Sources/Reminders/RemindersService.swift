...
final class RemindersService: RemindersServicing {
    func list(params: AutoCrabRemindersListParams) async throws -> AutoCrabRemindersListPayload {
        ...
        let payload: [AutoCrabReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return AutoCrabReminderPayload(
        ...
        return AutoCrabRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AutoCrabRemindersAddParams) async throws -> AutoCrabRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AutoCrabReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AutoCrabRemindersAddPayload(reminder: payload)
    }
