...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.autocrab.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.autocrab.ios", category: "PushWake")
    private let locationWakeLogger = Logger(subsystem: "ai.autocrab.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.autocrab.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = AutoCrabCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = AutoCrabCanvasA2UIAction.compactJSON(userAction["context"])
        ...
        let message = AutoCrabCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = AutoCrabCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: AutoCrabLocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(AutoCrabGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: AutoCrabNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: AutoCrabNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: AutoCrabNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(AutoCrabLocationGetParams.self, from: req.paramsJSON)) ??
            AutoCrabLocationGetParams()
        ...
        let payload = AutoCrabLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AutoCrabCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(AutoCrabCanvasPresentParams.self, from: req.paramsJSON)) ??
                AutoCrabCanvasPresentParams()
        ...
        case AutoCrabCanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case AutoCrabCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(AutoCrabCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case AutoCrabCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(AutoCrabCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case AutoCrabCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(AutoCrabCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case AutoCrabCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.autocrabA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing autocrabA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AutoCrabCanvasA2UICommand.push.rawValue, AutoCrabCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AutoCrabKit.AnyCodable]
            if command == AutoCrabCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(AutoCrabCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try AutoCrabCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(AutoCrabCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try AutoCrabCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.autocrabA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AutoCrabCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case AutoCrabCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(AutoCrabCameraSnapParams.self, from: req.paramsJSON)) ??
                AutoCrabCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case AutoCrabCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(AutoCrabCameraClipParams.self, from: req.paramsJSON)) ??
                AutoCrabCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(AutoCrabScreenRecordParams.self, from: req.paramsJSON)) ??
            AutoCrabScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(AutoCrabSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: AutoCrabNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: AutoCrabNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: AutoCrabNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(AutoCrabChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: AutoCrabNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = AutoCrabChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AutoCrabDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AutoCrabDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(AutoCrabPhotosLatestParams.self, from: req.paramsJSON)) ??
            AutoCrabPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AutoCrabContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(AutoCrabContactsSearchParams.self, from: req.paramsJSON)) ??
                AutoCrabContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AutoCrabContactsCommand.add.rawValue:
            let params = try Self.decodeParams(AutoCrabContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AutoCrabCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(AutoCrabCalendarEventsParams.self, from: req.paramsJSON)) ??
                AutoCrabCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AutoCrabCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(AutoCrabCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AutoCrabRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(AutoCrabRemindersListParams.self, from: req.paramsJSON)) ??
                AutoCrabRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AutoCrabRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(AutoCrabRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AutoCrabMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(AutoCrabMotionActivityParams.self, from: req.paramsJSON)) ??
                AutoCrabMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AutoCrabMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(AutoCrabPedometerParams.self, from: req.paramsJSON)) ??
                AutoCrabPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AutoCrabTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AutoCrabTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AutoCrabTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AutoCrabTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([AutoCrabLocationCommand.get.rawValue]) { [weak self] req in
        ...
        register([AutoCrabScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([AutoCrabSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([AutoCrabChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            AutoCrabWatchCommand.status.rawValue,
            AutoCrabWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([AutoCrabPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            AutoCrabRemindersCommand.list.rawValue,
            AutoCrabRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AutoCrabWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = AutoCrabWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AutoCrabWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(AutoCrabWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = AutoCrabWatchNotifyPayload(
    ...
    func locationMode() -> AutoCrabLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return AutoCrabLocationMode(rawValue: raw) ?? .off
    }
...
private extension NodeAppModel {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). "
                                        + "Approve on gateway and return to AutoCrab."
                            } else {
                                self.gatewayStatusText =
                                    "Pairing required. Approve on gateway and return to AutoCrab."
                            }
                        }
    ...
    func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "autocrab-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "AutoCrab Share Self-Test",
            url: URL(string: "https://autocrab.ai/share-self-test"),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.autocrabPushKind(userInfo)
        let receivedMessage =
    ...
    private static func autocrabPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["autocrab"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["autocrab"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
