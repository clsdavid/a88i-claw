...
@MainActor
final class AutoCrabAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.autocrab.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.autocrab.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.autocrab.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "autocrab.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "autocrab.watch.promptId"
    static let sessionKeyKey = "autocrab.watch.sessionKey"
    static let actionPrimaryIDKey = "autocrab.watch.action.primary.id"
    static let actionPrimaryLabelKey = "autocrab.watch.action.primary.label"
    static let actionSecondaryIDKey = "autocrab.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "autocrab.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "autocrab.watch.action.primary"
    static let actionSecondaryIdentifier = "autocrab.watch.action.secondary"
    static let actionIdentifierPrefix = "autocrab.watch.action."
    static let actionIDKeyPrefix = "autocrab.watch.action.id."
    static let actionLabelKeyPrefix = "autocrab.watch.action.label."
    static let categoryPrefix = "autocrab.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: AutoCrabWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> AutoCrabWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AutoCrabWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "AutoCrab" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [AutoCrabWatchAction]) -> [UNNotificationAction] {
...
@main
struct AutoCrabApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(AutoCrabAppDelegate.self) private var appDelegate
...
extension AutoCrabApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("AutoCrab: installing uncaught exception handler")
