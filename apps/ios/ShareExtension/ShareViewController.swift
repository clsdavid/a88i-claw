...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.autocrab.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to AutoCrab", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to AutoCrab gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to AutoCrab.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "AutoCrabShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "AutoCrab is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "AutoCrab Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("autocrab-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
