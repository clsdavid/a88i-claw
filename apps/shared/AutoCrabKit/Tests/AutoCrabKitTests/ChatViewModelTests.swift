...
import Foundation
import Testing
@testable import AutoCrabChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> AutoCrabChatHistoryPayload
{
    AutoCrabChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> AutoCrabChatSessionEntry {
...
private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [AutoCrabChatHistoryPayload],
    sessionsResponses: [AutoCrabChatSessionsListResponse] = []) async -> (TestChatTransport, AutoCrabChatViewModel)
{
    let transport = TestChatTransport(historyResponses: historyResponses, sessionsResponses: sessionsResponses)
    let vm = await MainActor.run { AutoCrabChatViewModel(sessionKey: sessionKey, transport: transport) }
    return (transport, vm)
}

private func loadAndWaitBootstrap(
    vm: AutoCrabChatViewModel,
...
private func sendUserMessage(_ vm: AutoCrabChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, AutoCrabChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [AutoCrabChatHistoryPayload]
    private let sessionsResponses: [AutoCrabChatSessionsListResponse]

    private let stream: AsyncStream<AutoCrabChatTransportEvent>
    private let continuation: AsyncStream<AutoCrabChatTransportEvent>.Continuation

    init(
        historyResponses: [AutoCrabChatHistoryPayload],
        sessionsResponses: [AutoCrabChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<AutoCrabChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> AutoCrabChatHistoryPayload {
        ...
        return self.historyResponses.last ?? AutoCrabChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [AutoCrabChatAttachmentPayload]) async throws -> AutoCrabChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return AutoCrabChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> AutoCrabChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? AutoCrabChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: AutoCrabChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = AutoCrabChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func stripsInboundMetadataFromHistoryMessages() async throws {
        let history = AutoCrabChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [
                AnyCodable([
...
"""]],
        ...
        let vm = await MainActor.run { AutoCrabChatViewModel(sessionKey: "main", transport: transport) }
