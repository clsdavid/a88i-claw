...
struct ChatMessageBubble: View {
    let message: AutoCrabChatMessage
    let style: AutoCrabChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? AutoCrabChatTheme.userText : AutoCrabChatTheme.assistantText
    ...
    private var inlineAttachments: [AutoCrabChatMessageContent] {
    ...
    private var toolCalls: [AutoCrabChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [AutoCrabChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? AutoCrabChatTheme.userBubble
        }
        if self.style == .onboarding {
            return AutoCrabChatTheme.onboardingAssistantBubble
        }
        return AutoCrabChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return AutoCrabChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: AutoCrabChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? AutoCrabChatTheme.userText : AutoCrabChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: AutoCrabChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AutoCrabChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(AutoCrabChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [AutoCrabChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: AutoCrabChatTheme.assistantText)
            }
        }
    }
}
