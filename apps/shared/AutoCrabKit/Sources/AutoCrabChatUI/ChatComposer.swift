...
@MainActor
struct AutoCrabChatComposer: View {
    @Bindable var viewModel: AutoCrabChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(AutoCrabChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(AutoCrabChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \AutoCrabPendingAttachment.id)
                { (att: AutoCrabPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            AutoCrabPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(AutoCrabChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AutoCrabChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(AutoCrabChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(AutoCrabChatTheme.subtleCard)
        .clipShape(Capsule())
    }
