...
@MainActor
public struct AutoCrabChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: AutoCrabChatViewModel
    ...
    public init(
        viewModel: AutoCrabChatViewModel,
    ...
    private var visibleMessages: [AutoCrabChatMessage] {
        let base: [AutoCrabChatMessage]
    ...
    private func mergeToolResults(in messages: [AutoCrabChatMessage]) -> [AutoCrabChatMessage] {
        ...
        for message in messages {
            ...
            let merged = AutoCrabChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: AutoCrabChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func toolCallIds(in message: AutoCrabChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: AutoCrabChatMessage) -> String {
