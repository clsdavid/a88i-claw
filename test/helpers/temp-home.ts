...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked AUTOCRAB_HOME.
  delete process.env.AUTOCRAB_HOME;
  process.env.AUTOCRAB_STATE_DIR = path.join(base, ".autocrab");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "autocrab-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".autocrab", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
