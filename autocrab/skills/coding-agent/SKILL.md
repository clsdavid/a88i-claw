...
metadata:
  {
    "autocrab": { "emoji": "🧩", "requires": { "anyBins": ["claude", "codex", "opencode", "pi"] } },
  }
---
...
**⚠️ CRITICAL: Never review PRs in AutoCrab's own project folder!**
...
8. **NEVER start Codex in ~/.autocrab/** - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/autocrab/** - that's the LIVE AutoCrab instance!
...
For long-running background tasks, append a wake trigger to your prompt so AutoCrab gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
autocrab system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: autocrab system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
