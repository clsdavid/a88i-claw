...
- `/usage cost` in chats: local cost summary aggregated from AutoCrab session logs.
- CLI: `autocrab status --usage` prints a full per-provider breakdown.
- CLI: `autocrab channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
