...
AutoCrab selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models AutoCrab can use (plus aliases).
...
`autocrab models` (no subcommand) is a shortcut for `models status`.
...
`autocrab models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.autocrab/agents/<agentId>/models.json`). This file
