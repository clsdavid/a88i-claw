...
Deploy AutoCrab on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/autocrab` — Control UI
...
- `AUTOCRAB_STATE_DIR=/data/.autocrab` (recommended)
- `AUTOCRAB_WORKSPACE_DIR=/data/workspace` (recommended)
- `AUTOCRAB_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your AutoCrab state + workspace so you can migrate to another host without losing config or memory.
