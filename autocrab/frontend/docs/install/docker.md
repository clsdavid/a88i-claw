---
summary: "Optional Docker-based setup and onboarding for AutoCrab"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run AutoCrab on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full AutoCrab in Docker)
...
- builds the gateway image locally (or pulls a remote image if `AUTOCRAB_IMAGE` is set)
...
- `AUTOCRAB_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/autocrab/autocrab:latest`)
- `AUTOCRAB_DOCKER_APT_PACKAGES` — install extra apt packages during build
- `AUTOCRAB_EXTRA_MOUNTS` — add extra host bind mounts
- `AUTOCRAB_HOME_VOLUME` — persist `/home/node` in a named volume
- `AUTOCRAB_SANDBOX` — opt in to Docker gateway sandbox bootstrap. Only explicit truthy values enable it: `1`, `true`, `yes`, `on`
- `AUTOCRAB_INSTALL_DOCKER_CLI` — build arg passthrough for local image builds (`1` installs Docker CLI in the image). `docker-setup.sh` sets this automatically when `AUTOCRAB_SANDBOX=1` for local builds.
- `AUTOCRAB_DOCKER_SOCKET` — override Docker socket path (default: `DOCKER_HOST=unix://...` path, else `/var/run/docker.sock`)
- `AUTOCRAB_ALLOW_INSECURE_PRIVATE_WS=1` — break-glass: allow trusted private-network
  `ws://` targets for CLI/onboarding client paths (default is loopback-only)
- `AUTOCRAB_BROWSER_DISABLE_GRAPHICS_FLAGS=0` — disable container browser hardening flags
  `--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu` when you need
  WebGL/3D compatibility.
- `AUTOCRAB_BROWSER_DISABLE_EXTENSIONS=0` — keep extensions enabled when browser
  flows require them (default keeps extensions disabled in sandbox browser).
- `AUTOCRAB_BROWSER_RENDERER_PROCESS_LIMIT=<N>` — set Chromium renderer process
...
- Need the URL again? Run `docker compose run --rm autocrab-cli dashboard --no-open`.
...
```bash
export AUTOCRAB_SANDBOX=1
export AUTOCRAB_DOCKER_SOCKET=/run/user/1000/docker.sock
./docker-setup.sh
```
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `autocrab-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
- For non-local `AUTOCRAB_IMAGE` values, the image must already contain Docker
...
```bash
docker compose run -T --rm autocrab-cli gateway probe
docker compose run -T --rm autocrab-cli devices list --json
```
...
`autocrab-cli` uses `network_mode: "service:autocrab-gateway"` so CLI commands can
...
network path instead of the bundled `autocrab-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `autocrab-cli`.
...
image instead, set `AUTOCRAB_IMAGE` before running the script:

```bash
export AUTOCRAB_IMAGE="ghcr.io/autocrab/autocrab:latest"
./docker-setup.sh
```

The script detects that `AUTOCRAB_IMAGE` is not the default `autocrab:local` and
...
`docker-compose.yml` and helper files. `AUTOCRAB_IMAGE` skips local image build
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/autocrab/autocrab/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```
...
```bash
docker build -t autocrab:local -f Dockerfile .
docker compose run --rm autocrab-cli onboard
docker compose up -d autocrab-gateway
```

Note: run `docker compose ...` from the repo root. If you enabled
`AUTOCRAB_EXTRA_MOUNTS` or `AUTOCRAB_HOME_VOLUME`, the setup script writes
...
```bash
docker compose run --rm autocrab-cli dashboard --no-open
docker compose run --rm autocrab-cli devices list
docker compose run --rm autocrab-cli devices approve <requestId>
```
...
`AUTOCRAB_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
`autocrab-gateway` and `autocrab-cli` by generating `docker-compose.extra.yml`.

Example:

```bash
export AUTOCRAB_EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
- If you edit `AUTOCRAB_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
volume via `AUTOCRAB_HOME_VOLUME`. This creates a Docker volume and mounts it at
`/home/node`, while keeping the standard config/workspace bind mounts. Use a
named volume here (not a bind path); for bind mounts, use
`AUTOCRAB_EXTRA_MOUNTS`.

Example:

```bash
export AUTOCRAB_HOME_VOLUME="autocrab_home"
./docker-setup.sh
```
...
- If you change `AUTOCRAB_HOME_VOLUME`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
libraries), set `AUTOCRAB_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
```bash
export AUTOCRAB_DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
- If you change `AUTOCRAB_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
```bash
export AUTOCRAB_DOCKER_APT_PACKAGES="git curl jq"
./docker-setup.sh
```
...
`AUTOCRAB_DOCKER_APT_PACKAGES` instead of using `--with-deps` at runtime.

4. **Persist Playwright browser downloads**:

- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `AUTOCRAB_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `AUTOCRAB_EXTRA_MOUNTS`.

### Permissions + EACCES

The image runs as `node` (uid 1000). If you see permission errors on
`/home/node/.autocrab`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/autocrab-config /path/to/autocrab-workspace
```
...
```bash
docker compose run --rm autocrab-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm autocrab-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm autocrab-cli channels add --channel discord --token "<token>"
```
...
background. In plain terms: Docker keeps checking if AutoCrab is still
...
```bash
docker compose exec autocrab-gateway node dist/index.js health --token "$AUTOCRAB_GATEWAY_TOKEN"
```
...
`docker-setup.sh` defaults `AUTOCRAB_GATEWAY_BIND=lan` so host access to
...
```bash
docker compose run --rm autocrab-cli config set gateway.mode local
docker compose run --rm autocrab-cli config set gateway.bind lan
docker compose run --rm autocrab-cli devices list --url ws://127.0.0.1:18789
```

### Notes

- Gateway bind defaults to `lan` for container use (`AUTOCRAB_GATEWAY_BIND`).
- Dockerfile CMD uses `--allow-unconfigured`; mounted config with `gateway.mode` not `local` will still start. Override CMD to enforce the guard.
- The gateway container is the source of truth for sessions (`~/.autocrab/agents/<agentId>/sessions/`).
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.autocrab/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  AutoCrab auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `autocrab sandbox recreate ...` command.
...
This builds `autocrab-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `autocrab-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "autocrab-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `autocrab-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`autocrab-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; AutoCrab provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
- Browser container startup defaults are conservative for shared/container workloads, including:
  ...
  - The three graphics hardening flags above are optional. If your workload needs
    WebGL/3D, set `AUTOCRAB_BROWSER_DISABLE_GRAPHICS_FLAGS=0` to run without
  ...
  - Extension behavior is controlled by `--disable-extensions` and can be disabled
    (enables extensions) via `AUTOCRAB_BROWSER_DISABLE_EXTENSIONS=0` for
    extension-dependent pages or extensions-heavy workflows.
  - `--renderer-process-limit=2` is also configurable with
    `AUTOCRAB_BROWSER_RENDERER_PROCESS_LIMIT`; set `0` to let Chromium choose its
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-autocrab-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-autocrab-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-autocrab-sbx" } },
    },
  },
}
```
...
- Custom tools not found: AutoCrab runs commands with `sh -lc` (login shell), which
