...
- Same path: run the app from a fixed location (for AutoCrab, `dist/AutoCrab.app`).
...
```bash
sudo tccutil reset Accessibility ai.autocrab.mac
sudo tccutil reset ScreenCapture ai.autocrab.mac
...
Workaround: move files into the AutoCrab workspace (`~/.autocrab/workspace`) if you want to avoid per-folder grants.
