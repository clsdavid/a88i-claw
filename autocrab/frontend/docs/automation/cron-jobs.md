...
- Jobs persist under `~/.autocrab/cron/` so restarts don’t lose schedules.
...
autocrab cron list
autocrab cron run <job-id>
autocrab cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.autocrab/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `autocrab cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, AutoCrab applies a
...
If `delivery` is omitted for isolated jobs, AutoCrab defaults to `announce`.
...
- CLI equivalents: `autocrab cron add --light-context ...` and `autocrab cron edit --light-context`.
...
- Job store: `~/.autocrab/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.autocrab/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, AutoCrab classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, AutoCrab archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `autocrab cron runs` and adjust retention before logs become large
...
autocrab cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
autocrab cron edit <jobId> --agent ops
autocrab cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
autocrab cron run <jobId>
autocrab cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
autocrab cron edit <jobId> \
...
```bash
autocrab cron edit <jobId> --exact
```
...
```bash
autocrab system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`autocrab system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `AUTOCRAB_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- AutoCrab applies exponential retry backoff for recurring jobs after consecutive errors:
