---
summary: "CLI reference for `autocrab onboard` (interactive onboarding wizard)"
read_when:
...
```bash
autocrab onboard
autocrab onboard --flow quickstart
autocrab onboard --flow manual
autocrab onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`AUTOCRAB_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `autocrab dashboard` (Control UI, no channel setup).
