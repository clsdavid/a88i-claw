...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in AutoCrab
...
Running AutoCrab via Node with `tsx` fails at startup with:

```
[autocrab] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `autocrab status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch autocrab.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node autocrab.mjs status` works on Node 25.
