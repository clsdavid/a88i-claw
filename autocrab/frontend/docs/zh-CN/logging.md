...
你可以在 `~/.autocrab/autocrab.json` 中覆盖此设置：
...
所有日志配置都在 `~/.autocrab/autocrab.json` 的 `logging` 下。
...
- AutoCrab 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `autocrab plugins enable diagnostics-otel` 启用插件。
...
- `autocrab.tokens`（计数器，属性：`autocrab.token`、`autocrab.channel`、`autocrab.provider`、`autocrab.model`）
- `autocrab.cost.usd`（计数器，属性：`autocrab.channel`、`autocrab.provider`、`autocrab.model`）
- `autocrab.run.duration_ms`（直方图，属性：`autocrab.channel`、`autocrab.provider`、`autocrab.model`）
- `autocrab.context.tokens`（直方图，属性：`autocrab.context`、`autocrab.channel`、`autocrab.provider`、`autocrab.model`）

消息流：

- `autocrab.webhook.received`（计数器，属性：`autocrab.channel`、`autocrab.webhook`）
- `autocrab.webhook.error`（计数器，属性：`autocrab.channel`、`autocrab.webhook`）
- `autocrab.webhook.duration_ms`（直方图，属性：`autocrab.channel`、`autocrab.webhook`）
- `autocrab.message.queued`（计数器，属性：`autocrab.channel`、`autocrab.source`）
- `autocrab.message.processed`（计数器，属性：`autocrab.channel`、`autocrab.outcome`）
- `autocrab.message.duration_ms`（直方图，属性：`autocrab.channel`、`autocrab.outcome`）

队列 + 会话：

- `autocrab.queue.lane.enqueue`（计数器，属性：`autocrab.lane`）
- `autocrab.queue.lane.dequeue`（计数器，属性：`autocrab.lane`）
- `autocrab.queue.depth`（直方图，属性：`autocrab.lane` 或 `autocrab.channel=heartbeat`）
- `autocrab.queue.wait_ms`（直方图，属性：`autocrab.lane`）
- `autocrab.session.state`（计数器，属性：`autocrab.state`、`autocrab.reason`）
- `autocrab.session.stuck`（计数器，属性：`autocrab.state`）
- `autocrab.session.stuck_age_ms`（直方图，属性：`autocrab.state`）
- `autocrab.run.attempt`（计数器，属性：`autocrab.attempt`）
...
- `autocrab.webhook.error`
  - `autocrab.channel`、`autocrab.webhook`、`autocrab.chatId`、`autocrab.error`
- `autocrab.message.processed`
  - `autocrab.channel`、`autocrab.outcome`、`autocrab.chatId`、`autocrab.messageId`、`autocrab.sessionKey`、`autocrab.sessionId`、`autocrab.reason`
- `autocrab.session.stuck`
  - `autocrab.state`、`autocrab.ageMs`、`autocrab.queueDepth`、`autocrab.sessionKey`、`autocrab.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `autocrab doctor`。
