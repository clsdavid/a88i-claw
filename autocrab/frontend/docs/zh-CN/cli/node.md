...
summary: "`autocrab node` 的 CLI 参考（无头节点主机）"
title: node
x-i18n:
...
```bash
autocrab node install --host <gateway-host> --port 18789
```
...
```bash
autocrab node status
autocrab node stop
autocrab node restart
autocrab node uninstall
```

使用 `autocrab node run` 运行前台节点主机（无服务）。
...
- `autocrab approvals --node <id|name|ip>`（从 Gateway 网关编辑）
