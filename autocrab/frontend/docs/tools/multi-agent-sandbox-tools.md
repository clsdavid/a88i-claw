...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `autocrab sandbox explain`.
...
Legacy `agent.*` configs are migrated by `autocrab doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=autocrab-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${AUTOCRAB_STATE_DIR:-$HOME/.autocrab}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
