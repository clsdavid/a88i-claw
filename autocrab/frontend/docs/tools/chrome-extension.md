---
summary: "Chrome extension: let AutoCrab drive your existing Chrome tab"
read_when:
...
The AutoCrab Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate autocrab-managed Chrome profile.
...
AutoCrab then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the AutoCrab release (npm package) as static files. There is no separate “build” step.

After upgrading AutoCrab:

- Re-run `autocrab browser extension install` to refresh the installed files under your AutoCrab state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

AutoCrab ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `AUTOCRAB_GATEWAY_TOKEN`)
...
- Open the tab you want AutoCrab to control.
...
- `ON`: attached; AutoCrab can drive that tab.
...
Debugging: `autocrab sandbox explain`
...
`autocrab browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `autocrab browser extension install` first to copy the extension to a stable location under your AutoCrab state directory.
...
- **This is not isolated** like the dedicated autocrab-managed profile.
