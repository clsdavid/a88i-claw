...
AutoCrab supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `AUTOCRAB_GATEWAY_TOKEN` (or `CLAWDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, AutoCrab runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). AutoCrab validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/AutoCrab QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/autocrab"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `autocrab.json` refs)
...
When reload-time activation fails after a healthy state, AutoCrab enters degraded secrets state.
...
- Strict command paths (for example `autocrab memory` remote-memory paths and `autocrab qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `autocrab status`, `autocrab status --all`, `autocrab channels status`, `autocrab channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `autocrab secrets reload`.
...
- plaintext values at rest (`autocrab.json`, `auth-profiles.json`, `.env`)
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `autocrab.json` refs)
...
- lets you select supported secret-bearing fields in `autocrab.json` plus `auth-profiles.json` for one agent scope
...
- `autocrab secrets configure --agent <id>`
...
AutoCrab intentionally does not write rollback backups containing historical plaintext secret values.
