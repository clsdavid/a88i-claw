...
read_when:
  - Running AutoCrab behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of AutoCrab
...
- You run AutoCrab behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. AutoCrab checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. AutoCrab extracts the user identity from the configured header
...
- AutoCrab can stay on loopback HTTP behind the proxy.
...
If AutoCrab itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://autocrab.example.com
    to: http://autocrab-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
autocrab.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy autocrab:18789 {
...
location / {
    ...
    proxy_pass http://autocrab:18789;
...
`autocrab security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update AutoCrab config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `autocrab security audit` and review findings
