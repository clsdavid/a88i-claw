...
Tlon is a decentralized messenger built on Urbit. AutoCrab connects to your Urbit ship and can
...
```bash
autocrab plugins install @autocrab/tlon
```

Local checkout (when running from a git repo):

```bash
autocrab plugins install ./extensions/tlon
```
...
By default, AutoCrab blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `autocrab message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, AutoCrab replies in-thread.
