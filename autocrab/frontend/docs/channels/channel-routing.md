...
AutoCrab routes replies **back to the channel where a message came from**. The
...
AutoCrab infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, AutoCrab still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when AutoCrab would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.autocrab/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.autocrab`):

- `~/.autocrab/agents/<agentId>/sessions/sessions.json`
