...
Matrix is an open, decentralized messaging protocol. AutoCrab connects as a Matrix **user**
...
```bash
autocrab plugins install @autocrab/matrix
```

Local checkout (when running from a git repo):

```bash
autocrab plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `autocrab plugins install @autocrab/matrix`
   - From a local checkout: `autocrab plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: AutoCrab calls the same
     login endpoint, stores the access token in `~/.autocrab/credentials/matrix/credentials.json`,
...
- On first connection, AutoCrab requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  AutoCrab logs a warning.
...
`~/.autocrab/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, AutoCrab resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
