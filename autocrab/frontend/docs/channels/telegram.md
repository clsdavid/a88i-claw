...
```
    ...
    Telegram does **not** use `autocrab channels login telegram`; configure token in config/env, then start gateway.
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `autocrab doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `autocrab doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `autocrab logs --follow`
...
- DM messages can carry `message_thread_id`; AutoCrab routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (native drafts + message edits)">
    AutoCrab can stream partial replies in real time:
    ...
    - DM: AutoCrab updates the draft in place (no extra preview message)
    - group/topic: AutoCrab keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), AutoCrab falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, AutoCrab skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, AutoCrab automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, AutoCrab retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - AutoCrab pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.autocrab/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, AutoCrab enqueues system events like:
...
```bash
autocrab message send --channel telegram --target 123456789 --message "hi"
autocrab message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `autocrab message poll` and support forum topics:

```bash
autocrab message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
autocrab message poll --channel telegram --target -1001234567890:topic:42 \
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `autocrab channels status` warns when config expects unmentioned group messages.
    - `autocrab channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `autocrab logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, AutoCrab now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `AUTOCRAB_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `AUTOCRAB_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `AUTOCRAB_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `autocrab doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `autocrab doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, AutoCrab falls back to the first normalized account ID and `autocrab doctor` warns.
