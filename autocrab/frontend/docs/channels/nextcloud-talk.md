...
```bash
autocrab plugins install @autocrab/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
autocrab plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "AutoCrab" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `autocrab pairing list nextcloud-talk`
  - `autocrab pairing approve nextcloud-talk <CODE>`
