---
summary: "Mattermost bot setup and AutoCrab config"
read_when:
...
```bash
autocrab plugins install @autocrab/mattermost
```

Local checkout (when running from a git repo):

```bash
autocrab plugins install ./extensions/mattermost
```
...
4. Configure AutoCrab and start the gateway.
...
Native slash commands are opt-in. When enabled, AutoCrab registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, AutoCrab derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as AutoCrab.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to AutoCrab.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from AutoCrab, not `404`.
...
- Approve via:
  - `autocrab pairing list mattermost`
  - `autocrab pairing approve mattermost <CODE>`
...
Use these target formats with `autocrab message send` or cron/webhooks:
...
- If `interactions.callbackBaseUrl` is omitted, AutoCrab derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and AutoCrab run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="autocrab-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"autocrab-mattermost-interactions",
...
`autocrab message send` and cron/webhook deliveries.
