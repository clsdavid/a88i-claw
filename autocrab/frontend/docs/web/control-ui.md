...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/autocrab`)
...
If the page fails to load, start the Gateway first: `autocrab gateway`.
...
you revoke it with `autocrab devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.autocrab/autocrab.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop autocrab`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. AutoCrab
...
AutoCrab **blocks** Control UI connections without device identity.
