...
```bash
autocrab tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `autocrab tui --deliver`
...
- Local shell commands receive `AUTOCRAB_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `autocrab logs --follow`.
- Confirm the agent can run: `autocrab status` and `autocrab models status`.
...
- No agents in picker: check `autocrab agents list` and your routing config.
