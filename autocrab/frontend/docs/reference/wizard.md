...
This is the full reference for the `autocrab onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.autocrab/credentials/oauth.json`; auth profiles live in `~/.autocrab/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.autocrab/credentials/oauth.json` (or `$AUTOCRAB_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.autocrab/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `autocrab pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `autocrab health`.
    - Tip: `autocrab status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export AUTOCRAB_GATEWAY_TOKEN="your-token"
autocrab onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env AUTOCRAB_GATEWAY_TOKEN
```
...
- Stores it under `~/.autocrab/tools/signal-cli/<version>/`.
...
Typical fields in `~/.autocrab/autocrab.json`:
...
`autocrab agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.autocrab/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.autocrab/agents/<agentId>/sessions/`.
