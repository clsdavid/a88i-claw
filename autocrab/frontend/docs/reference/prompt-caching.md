...
- With Anthropic API-key auth profiles, AutoCrab seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, AutoCrab injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
AutoCrab exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.autocrab/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$AUTOCRAB_STATE_DIR/logs/cache-trace.jsonl`
...
- `AUTOCRAB_CACHE_TRACE=1` enables cache tracing.
- `AUTOCRAB_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `AUTOCRAB_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `AUTOCRAB_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `AUTOCRAB_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
