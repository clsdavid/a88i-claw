---
summary: "How AutoCrab builds prompt context and reports token usage + costs"
read_when:
...
AutoCrab tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

AutoCrab assembles its own system prompt on every run. It includes:
...
For images, AutoCrab downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from AutoCrab session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `autocrab status --usage` and `autocrab channels list` show
...
`cacheWrite`. If pricing is missing, AutoCrab shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. AutoCrab can
...
Anthropic's 1M context window is currently beta-gated. AutoCrab can inject the
...
AutoCrab skips the `context-1m-*` beta header because Anthropic currently
