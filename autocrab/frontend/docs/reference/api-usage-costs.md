...
AutoCrab features that can generate provider usage or paid API calls.
...
- `autocrab status --usage` and `autocrab channels list` show provider **usage windows**
...
AutoCrab can pick up credentials from:
...
- `autocrab status --usage`
- `autocrab models status --json`
...
`autocrab models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
