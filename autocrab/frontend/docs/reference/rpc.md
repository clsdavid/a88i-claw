...
AutoCrab integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- AutoCrab owns lifecycle when `channels.signal.autoStart=true`.
...
- AutoCrab spawns `imsg rpc` as a child process (legacy iMessage integration).
