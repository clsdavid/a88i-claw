...
export async function handleWhatsAppStart(host: AutoCrabApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: AutoCrabApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: AutoCrabApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: AutoCrabApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: AutoCrabApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: AutoCrabApp): string {
...
function resolveGatewayHttpAuthHeader(host: AutoCrabApp): string | null {
...
function buildGatewayHttpHeaders(host: AutoCrabApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: AutoCrabApp) {
...
export function handleNostrProfileToggleAdvanced(host: AutoCrabApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: AutoCrabApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: AutoCrabApp) {
  const state = host.nostrProfileFormState;
