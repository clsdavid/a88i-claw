...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__AUTOCRAB_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<AutoCrabTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.autocrab", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = AutoCrabTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> AutoCrabTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return AutoCrabTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> AutoCrabTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> AutoCrabTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return AutoCrabTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> AutoCrabTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = AutoCrabTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: AutoCrabTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: AutoCrabAgentEventPayload.self
            ) else {
                continue
            }
