...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> AutoCrabDeviceStatusPayload {
        ...
        return AutoCrabDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> AutoCrabDeviceInfoPayload {
        ...
        return AutoCrabDeviceInfoPayload(
    ...
    private func batteryStatus() -> AutoCrabBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: AutoCrabBatteryState = switch device.batteryState {
        ...
        return AutoCrabBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> AutoCrabThermalStatusPayload {
        let state: AutoCrabThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return AutoCrabThermalStatusPayload(state: state)
    }

    private func storageStatus() -> AutoCrabStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return AutoCrabStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
