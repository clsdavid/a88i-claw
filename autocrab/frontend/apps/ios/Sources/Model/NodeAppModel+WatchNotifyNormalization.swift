...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: AutoCrabWatchNotifyParams) -> AutoCrabWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [AutoCrabWatchAction]?,
        kind: String?,
        promptId: String?) -> [AutoCrabWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> AutoCrabWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AutoCrabWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                AutoCrabWatchAction(id: "approve", label: "Approve"),
                AutoCrabWatchAction(id: "decline", label: "Decline", style: "destructive"),
                AutoCrabWatchAction(id: "open_phone", label: "Open iPhone"),
                AutoCrabWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            AutoCrabWatchAction(id: "done", label: "Done"),
            AutoCrabWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            AutoCrabWatchAction(id: "open_phone", label: "Open iPhone"),
            AutoCrabWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: AutoCrabWatchRisk?,
        priority: AutoCrabNotificationPriority?) -> AutoCrabWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: AutoCrabNotificationPriority?,
        risk: AutoCrabWatchRisk?) -> AutoCrabNotificationPriority?
    {
