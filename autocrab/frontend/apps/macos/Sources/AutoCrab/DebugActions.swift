...
enum DebugActions {
    private static let verboseDefaultsKey = "autocrab.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "autocrab.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AutoCrabPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "AutoCrab", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AutoCrabPaths.configURL
        guard
