...
enum CommandResolver {
    private static let projectRootDefaultsKey = "autocrab.gatewayProjectRootPath"
    private static let helperName = "autocrab"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let autocrabEntry = root.appendingPathComponent("autocrab.mjs").path
        if FileManager().isReadableFile(atPath: autocrabEntry) { return autocrabEntry }
        let binEntry = root.appendingPathComponent("bin/autocrab.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/autocrab")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let autocrabPaths = self.autocrabManagedPaths(home: home)
        if !autocrabPaths.isEmpty {
            extras.insert(contentsOf: autocrabPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + autocrabPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func autocrabManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".autocrab"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func autocrabExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAutoCrabExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("autocrab.mjs").path,
            root.appendingPathComponent("bin/autocrab.js").path,
        ]
    ...
    static func hasAnyAutoCrabInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.autocrabExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func autocrabNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let autocrabPath = self.projectAutoCrabExecutable(projectRoot: root) {
            return [autocrabPath, subcommand] + extraArgs
        }
        if let autocrabPath = self.autocrabExecutable(searchPaths: searchPaths) {
            return [autocrabPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "autocrab", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            autocrab entrypoint missing (looked for dist/index.js or autocrab.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real autocrab CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/autocrab"
        ...
        \(cliSection)
        \(projectSection)
        if command -v autocrab >/dev/null 2>&1; then
          CLI="$(command -v autocrab)"
          autocrab \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/autocrab.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/autocrab.mjs"
            node "$PRJ/autocrab.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/autocrab.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/autocrab.js"
            node "$PRJ/bin/autocrab.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent autocrab"
          pnpm --silent autocrab \(quotedArgs);
        else
          echo "autocrab CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? AutoCrabConfigFile.loadDict()
