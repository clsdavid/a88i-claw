...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.autocrab", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: AutoCrabNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [AutoCrabCapability.canvas.rawValue, AutoCrabCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(AutoCrabCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if AutoCrabLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(AutoCrabCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            AutoCrabSystemCommand.which.rawValue,
            AutoCrabSystemCommand.run.rawValue,
            AutoCrabSystemCommand.execApprovalsGet.rawValue,
            AutoCrabSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(AutoCrabCapability.camera.rawValue) {
        ...
        if capsSet.contains(AutoCrabCapability.location.rawValue) {
