...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.autocrab", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AutoCrabSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AutoCrabSessionPreviewEntry,
    ...
    private static func cache(payload: AutoCrabSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AutoCrabChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AutoCrabKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AutoCrabKit.AnyCodable]) -> [AutoCrabChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AutoCrabChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AutoCrabChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AutoCrabChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AutoCrabChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AutoCrabChatMessage) -> String? {
