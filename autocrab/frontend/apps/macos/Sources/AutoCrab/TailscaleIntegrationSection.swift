...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.autocrab/autocrab.json. Prefer AUTOCRAB_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.autocrab/autocrab.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.autocrab/autocrab.json. Restart the gateway to apply."
        }
