...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.autocrab", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous AutoCrab run already has an SSH listener on the expected port (common after restarts),
