...
private let deepLinkLogger = Logger(subsystem: "ai.autocrab", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "AutoCrab is paused", message: "Unpause AutoCrab to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run AutoCrab agent?", message: body) else { return }
        }
