...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.autocrab", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.autocrab", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [AutoCrabChatMessage] = messages.compactMap { item in
