...
enum AutoCrabLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = AutoCrabOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AutoCrabFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AutoCrabLogging.bootstrapIfNeeded()
        let label = AutoCrabLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AutoCrabOSLogHandler: AppLogLevelBackedHandler {
...
struct AutoCrabFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AutoCrabLogging.parseLabel(self.label)
