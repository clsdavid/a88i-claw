...
struct GatewayDiscoveryModelTests {
    ...
    @Test func localGatewayMatchesDisplayName() {
        ...
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: "Peter's Mac Studio (AutoCrab)",
            serviceName: nil,
            local: local))
    }
    ...
    @Test func serviceNameDoesNotFalsePositiveOnSubstringHostToken() {
        ...
        #expect(!GatewayDiscoveryModel.isLocalGateway(
            ...
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipetacstudio (AutoCrab)",
            local: local))
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipete (AutoCrab)",
            local: local))
    }
    ...
    @Test func parsesGatewayTXTFields() {
        ...
        #expect(parsed.cliPath == "/opt/autocrab")
    }
    ...
    @Test func dedupeKeyPrefersResolvedEndpointAcrossSources() {
        let wideArea = GatewayDiscoveryModel.DiscoveredGateway(
            ...
            gatewayPort: 443,
            cliPath: nil,
            stableID: "wide-area|autocrab.internal.|gateway-host",
            debugID: "wide-area",
            isLocal: false)
