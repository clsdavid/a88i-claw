...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    private func voiceWakeChangedEvent(payload: AutoCrabProtocol.AnyCodable) -> EventFrame {
        EventFrame(
            type: "event",
    ...
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let evt = voiceWakeChangedEvent(payload: AutoCrabProtocol.AnyCodable(["triggers": ["autocrab", "computer"]]))
        ...
        #expect(updated == ["autocrab", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        let previous = await applyTriggersAndCapturePrevious(["before"])
        let evt = voiceWakeChangedEvent(payload: AutoCrabProtocol.AnyCodable(["unexpected": 123]))
