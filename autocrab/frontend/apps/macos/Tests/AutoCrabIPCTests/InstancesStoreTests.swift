...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: AutoCrabProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: AutoCrabProtocol.AnyCodable] = [
            "presence": .init([AutoCrabProtocol.AnyCodable(entry)]),
        ]
        let payload = AutoCrabProtocol.AnyCodable(payloadMap)
