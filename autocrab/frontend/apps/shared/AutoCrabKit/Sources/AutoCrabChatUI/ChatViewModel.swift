...
private let chatUILogger = Logger(subsystem: "ai.autocrab", category: "AutoCrabChatUI")

@MainActor
@Observable
public final class AutoCrabChatViewModel {
    public private(set) var messages: [AutoCrabChatMessage] = []
    ...
    public var attachments: [AutoCrabPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [AutoCrabChatPendingToolCall] = []
    public private(set) var sessions: [AutoCrabChatSessionEntry] = []
    private let transport: any AutoCrabChatTransport
    ...
    private var pendingToolCallsById: [String: AutoCrabChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any AutoCrabChatTransport) {
    ...
    public var sessionChoices: [AutoCrabChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [AutoCrabChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: AutoCrabPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [AutoCrabChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AutoCrabChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: AutoCrabChatMessage) -> AutoCrabChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> AutoCrabChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return AutoCrabChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: AutoCrabChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [AutoCrabChatMessage],
        incoming: [AutoCrabChatMessage]) -> [AutoCrabChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [AutoCrabChatMessage]) -> [AutoCrabChatMessage] {
        var result: [AutoCrabChatMessage] = []
    ...
    private static func dedupeKey(for message: AutoCrabChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [AutoCrabChatMessageContent] = [
            AutoCrabChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> AutoCrabChatAttachmentPayload in
            AutoCrabChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> AutoCrabChatSessionEntry {
        AutoCrabChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: AutoCrabChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: AutoCrabChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: AutoCrabAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AutoCrabChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> AutoCrabPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
