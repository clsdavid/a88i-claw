...
enum AutoCrabChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("AutoCrabChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("AutoCrabChatTheme.onboardingAssistantBubble"),
...
enum AutoCrabPlatformImageFactory {
    static func image(_ image: AutoCrabPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
