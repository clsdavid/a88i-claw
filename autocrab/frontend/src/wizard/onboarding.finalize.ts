...
type FinalizeOnboardingOptions = {
  flow: WizardFlow;
  opts: OnboardOptions;
  baseConfig: AutoCrabConfig;
  nextConfig: AutoCrabConfig;
  workspaceDir: string;
...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.autocrab.ai/gateway/health",
          "https://docs.autocrab.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.autocrab.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.autocrab/autocrab.json (gateway.auth.token) or AUTOCRAB_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("autocrab config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("autocrab doctor --generate-gateway-token")}`,
        "Web UI stores a copy in this browser's localStorage (autocrab.control.settings.v1).",
        `Open the dashboard anytime: ${formatCliCommand("autocrab dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control AutoCrab."
            : "Copy/paste this URL in a browser on this machine to control AutoCrab.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("autocrab dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.autocrab.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.autocrab.ai/security",
    "Security",
  );
  ...
  await prompter.note(
    'What now: https://autocrab.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control AutoCrab."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control AutoCrab.",
  );
