...
const readConfigFileSnapshot = vi.hoisted(() =>
  vi.fn(async () => ({
    path: "/tmp/.autocrab/autocrab.json",
    exists: false,
...
vi.mock("../commands/onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "/tmp/autocrab-workspace",
...
describe("runOnboardingWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-onboard-suite-"));
  });
  ...
  it("resolves gateway.auth.password SecretRef for local onboarding probe", async () => {
    ...
    process.env.AUTOCRAB_GATEWAY_PASSWORD = "gateway-ref-password";
