...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "AutoCrab is a hobby project and still in beta. Expect sharp edges.",
      "By default, AutoCrab is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "AutoCrab is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run AutoCrab.",
      ...
      "",
      "Must read: https://docs.autocrab.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("AutoCrab onboarding");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: AutoCrabConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.autocrab.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("autocrab doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("autocrab configure")}.`;
  ...
  let localGatewayToken = process.env.AUTOCRAB_GATEWAY_TOKEN ?? process.env.CLAWDBOT_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword =
    process.env.AUTOCRAB_GATEWAY_PASSWORD ?? process.env.CLAWDBOT_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: AutoCrabConfig = applyOnboardingLocalWorkspaceConfig(baseConfig, workspaceDir);
