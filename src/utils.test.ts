...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("autocrab-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("autocrab-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("autocrab-lid-a-", (first) => {
      withTempDirSync("autocrab-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.autocrab when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "autocrab-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers AUTOCRAB_HOME over HOME", () => {
    vi.stubEnv("AUTOCRAB_HOME", "/srv/autocrab-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/autocrab-home"));
...
describe("shortenHomePath", () => {
  it("uses $AUTOCRAB_HOME prefix when AUTOCRAB_HOME is set", () => {
    vi.stubEnv("AUTOCRAB_HOME", "/srv/autocrab-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/autocrab-home")}/.autocrab/autocrab.json`)).toBe(
      "$AUTOCRAB_HOME/.autocrab/autocrab.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $AUTOCRAB_HOME replacement when AUTOCRAB_HOME is set", () => {
    vi.stubEnv("AUTOCRAB_HOME", "/srv/autocrab-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/autocrab-home")}/.autocrab/autocrab.json`),
    ).toBe("config: $AUTOCRAB_HOME/.autocrab/autocrab.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/autocrab")).toBe(path.resolve(os.homedir(), "autocrab"));
  });
  ...
  it("prefers AUTOCRAB_HOME for tilde expansion", () => {
    vi.stubEnv("AUTOCRAB_HOME", "/srv/autocrab-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/autocrab")).toBe(path.resolve("/srv/autocrab-home", "autocrab"));
