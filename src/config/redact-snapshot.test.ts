...
import type { ConfigFileSnapshot } from "./types.autocrab.js";
import { AutoCrabSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(AutoCrabSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.autocrab/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/autocrab/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/autocrab/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/autocrab/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/autocrab/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = AutoCrabSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "AutoCrabConfig";
    const hints = mainSchemaHints;
