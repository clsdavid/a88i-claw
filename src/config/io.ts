...
import type { AutoCrabConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareAutoCrabVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  autocrab config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  autocrab config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: AutoCrabConfig,
  pathSegments: string[],
): { changed: boolean; next: AutoCrabConfig } {
...
function coerceConfig(value: unknown): AutoCrabConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AutoCrabConfig;
}
...
function stampConfigVersion(cfg: AutoCrabConfig): AutoCrabConfig {
...
function warnIfConfigFromFuture(cfg: AutoCrabConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareAutoCrabVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer AutoCrab (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as AutoCrabConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AutoCrabConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AutoCrabConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AutoCrabConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as AutoCrabConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.AUTOCRAB_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.AUTOCRAB_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.AUTOCRAB_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.AUTOCRAB_WATCH_SESSION === "string" &&
        deps.env.AUTOCRAB_WATCH_SESSION.trim().length > 0
          ? deps.env.AUTOCRAB_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.AUTOCRAB_WATCH_COMMAND === "string" &&
        deps.env.AUTOCRAB_WATCH_COMMAND.trim().length > 0
          ? deps.env.AUTOCRAB_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `AUTOCRAB_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: AutoCrabConfig | null = null;
let runtimeConfigSourceSnapshot: AutoCrabConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AUTOCRAB_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.AUTOCRAB_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): AutoCrabConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): AutoCrabConfig {
