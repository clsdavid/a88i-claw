...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "autocrab.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    AUTOCRAB_PLUGIN_MANIFEST_CACHE_MS: process.env.AUTOCRAB_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-config-plugin-validation-"));
    ...
    process.env.AUTOCRAB_STATE_DIR = path.join(suiteHome, ".autocrab");
    process.env.AUTOCRAB_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.AUTOCRAB_STATE_DIR === undefined) {
      delete process.env.AUTOCRAB_STATE_DIR;
    } else {
      process.env.AUTOCRAB_STATE_DIR = envSnapshot.AUTOCRAB_STATE_DIR;
    }
    if (envSnapshot.AUTOCRAB_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.AUTOCRAB_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.AUTOCRAB_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.AUTOCRAB_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
