...
describe("config io write", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-config-io-"));
  });
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".autocrab", "autocrab.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".autocrab", "logs", "config-audit.jsonl");
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withSuiteHome(async (home) => {
      ...
      const invalidConfig: AutoCrabConfig = {
        channels: {
          telegram: {
            dmPolicy: "open",
            allowFrom: [],
      ...
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        "autocrab config set channels.telegram.allowFrom '[\"*\"]'",
      );
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        'autocrab config set channels.telegram.dmPolicy "pairing"',
      );
    });
  });
  ...
  it("does not mutate caller config when unsetPaths is applied on first write", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".autocrab", "autocrab.json");
      const io = createConfigIO({
        env: {} as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withSuiteHome(async (home) => {
      const { last } = await writeGatewayPatchAndReadLastAuditEntry({
        ...
        env: {
          AUTOCRAB_WATCH_MODE: "1",
          AUTOCRAB_WATCH_SESSION: "watch-session-1",
          AUTOCRAB_WATCH_COMMAND: "gateway --force",
