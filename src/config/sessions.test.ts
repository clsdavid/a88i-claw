...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ AUTOCRAB_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-autocrab",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-autocrab#general");
  });
  ...
  it("derives session transcripts dir from AUTOCRAB_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { AUTOCRAB_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when AUTOCRAB_STATE_DIR differs from stored paths", () => {
