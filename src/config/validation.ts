...
import type { AutoCrabConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AutoCrabConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: AutoCrabConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: AutoCrabConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AutoCrabSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AutoCrabConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AutoCrabConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as AutoCrabConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as AutoCrabConfig,
  };
}
