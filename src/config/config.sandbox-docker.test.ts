...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("autocrab-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "autocrab-sandbox-browser-global" },
      agentBrowser: { network: "autocrab-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("autocrab-sandbox-browser-agent");
  });
