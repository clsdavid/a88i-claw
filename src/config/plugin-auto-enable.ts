...
function isWhatsAppConfigured(cfg: AutoCrabConfig): boolean {
...
function isGenericChannelConfigured(cfg: AutoCrabConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: AutoCrabConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AutoCrabConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: AutoCrabConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: AutoCrabConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: AutoCrabConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: AutoCrabConfig, pluginId: string): AutoCrabConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
