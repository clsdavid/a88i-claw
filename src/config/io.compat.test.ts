...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-config-"));
  try {
    await run(home);
  } finally {
...
describe("config io paths", () => {
  it("uses ~/.autocrab/autocrab.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".autocrab", 19001);
  ...
  it("defaults to ~/.autocrab/autocrab.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".autocrab", "autocrab.json"));
    });
  });

  it("uses AUTOCRAB_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { AUTOCRAB_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".autocrab", "autocrab.json"));
    });
  });

  it("honors explicit AUTOCRAB_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".autocrab", 20002, "custom.json");
      const io = createIoForHome(home, { AUTOCRAB_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("honors legacy CLAWDBOT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".autocrab", 20003, "legacy-custom.json");
  ...
  it("normalizes safe-bin config entries at config load time", async () => {
    await withTempHome(async (home) => {
      ...
      const configPath = path.join(configDir, "autocrab.json");
      await fs.writeFile(
        configPath,
        JSON.stringify(
          {
