...
describe("resolveEffectiveHomeDir", () => {
  it("prefers AUTOCRAB_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/autocrab-home"),
    );
  });
  ...
  it("expands AUTOCRAB_HOME when set to ~", () => {
    const env = {
      AUTOCRAB_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for AUTOCRAB_HOME", () => {
    const result = resolveRequiredHomeDir(
      { AUTOCRAB_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when AUTOCRAB_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ AUTOCRAB_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { AUTOCRAB_HOME: "/srv/autocrab-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/autocrab-home")}/x`);
  });
