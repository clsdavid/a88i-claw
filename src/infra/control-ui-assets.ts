...
import { resolveAutoCrabPackageRoot, resolveAutoCrabPackageRootSync } from "./autocrab-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveAutoCrabPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "autocrab" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveAutoCrabPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // autocrab.mjs or dist/<bundle>.js
