...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["AUTOCRAB_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.AUTOCRAB_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveAutoCrabPackageRoot } = await import("./autocrab-root.js"));
    ...
    vi.mocked(resolveAutoCrabPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveAutoCrabPackageRoot).mockResolvedValue("/opt/autocrab");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/autocrab/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
