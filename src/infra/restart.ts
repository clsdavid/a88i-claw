...
export function triggerAutoCrabRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.AUTOCRAB_SYSTEMD_UNIT,
        process.env.AUTOCRAB_PROFILE,
      );
  ...
  const label =
    process.env.AUTOCRAB_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.AUTOCRAB_PROFILE);
