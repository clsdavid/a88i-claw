...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred AutoCrab tmp root", async () => {
    const tmpRoot = resolvePreferredAutoCrabTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideAutoCrab = path.join(os.tmpdir(), "outside-autocrab", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
