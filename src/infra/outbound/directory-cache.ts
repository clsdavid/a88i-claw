...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AutoCrabConfig | null = null;
  ...
  get(key: string, cfg: AutoCrabConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AutoCrabConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: AutoCrabConfig): void {
