...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses autocrab-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "autocrab-gateway" },
          { pid: process.pid, cmd: "autocrab-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'autocrab'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple autocrab pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "autocrab-gateway" },
          { pid: pid2, cmd: "autocrab-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-autocrab pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks autocrab processes — non-autocrab listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 1) {
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: stalePid, cmd: "autocrab-gateway" }]),
            stderr: "",
          };
        }
    ...
    it("lsof status 1 with non-empty autocrab stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty autocrab stdout must not be treated as port-free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 2) {
          // status 1 + autocrab pid in stdout — container-restricted lsof reports partial results
          return {
            error: null,
            status: 1,
            stdout: lsofOutput([{ pid: stalePid, cmd: "autocrab-gateway" }]),
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'autocrab'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("autocrab")
      // inside the mid-loop flush: a non-autocrab cmd between two entries must not
      // be pushed, but the following autocrab entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-autocrab entry first, then autocrab entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid autocrab entry must still be found.
      const stdout = `p0\ncautocrab-gateway\np${stalePid}\ncautocrab-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero autocrab pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-autocrab stdout (line 145)", () => {
    it("treats status 1 + non-autocrab stdout as port-free (not an autocrab process)", () => {
      // status 1 + non-empty stdout where no autocrab pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill autocrab pids) it is effectively free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        // status 1 + non-autocrab output — should be treated as free:true for our purposes
        return {
          error: null,
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no autocrab pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
