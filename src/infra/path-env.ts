...
function candidateBinDirs(opts: EnsureAutoCrabPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `autocrab` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "autocrab");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.AUTOCRAB_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "autocrab"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `autocrab` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAutoCrabCliOnPath(opts: EnsureAutoCrabPathOpts = {}) {
  if (isTruthyEnvValue(process.env.AUTOCRAB_PATH_BOOTSTRAPPED)) {
    return;
  }
