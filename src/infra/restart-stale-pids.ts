 ...
 * returning control to the caller (typically `triggerAutoCrabRestart` →
...
/**
 * Parse autocrab gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("autocrab")) {
 ...
 * Returns only PIDs that belong to autocrab gateway processes (not the current process).
 */
