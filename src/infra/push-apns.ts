...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.AUTOCRAB_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.AUTOCRAB_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set AUTOCRAB_APNS_TEAM_ID and AUTOCRAB_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.AUTOCRAB_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.AUTOCRAB_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.AUTOCRAB_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set AUTOCRAB_APNS_PRIVATE_KEY_P8 or AUTOCRAB_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading AUTOCRAB_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createAutoCrabPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    autocrab: createAutoCrabPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
