...
const DEFAULT_PACKAGE_NAME = "autocrab";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["autocrab", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "autocrab.mjs");
    ...
    const doctorStep = await runStep(
      step("autocrab doctor", doctorArgv, gitRoot, { AUTOCRAB_UPDATE_IN_PROGRESS: "1" }),
    );
