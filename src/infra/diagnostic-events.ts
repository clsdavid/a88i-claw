...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __autocrabDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__autocrabDiagnosticEventsState) {
    globalStore.__autocrabDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__autocrabDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: AutoCrabConfig): boolean {
