...
import { POSIX_AUTOCRAB_TMP_DIR, resolvePreferredAutoCrabTmpDir } from "./tmp-autocrab-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredAutoCrabTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `autocrab-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredAutoCrabTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_AUTOCRAB_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredAutoCrabTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredAutoCrabTmpDir", () => {
  it("prefers /tmp/autocrab when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_AUTOCRAB_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/autocrab when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_AUTOCRAB_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/autocrab when /tmp/autocrab is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/autocrab when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/autocrab is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/autocrab is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/autocrab is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback AutoCrab temp dir/);
  });
