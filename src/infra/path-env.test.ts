...
let ensureAutoCrabCliOnPath: typeof import("./path-env.js").ensureAutoCrabCliOnPath;

describe("ensureAutoCrabCliOnPath", () => {
  const envKeys = [
    "PATH",
    "AUTOCRAB_PATH_BOOTSTRAPPED",
    "AUTOCRAB_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureAutoCrabCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling autocrab exists", () => {
    const tmp = abs("/tmp/autocrab-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "autocrab");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.AUTOCRAB_PATH_BOOTSTRAPPED;

    ensureAutoCrabCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.AUTOCRAB_PATH_BOOTSTRAPPED = "1";
    ensureAutoCrabCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/autocrab-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "autocrab");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/autocrab-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "autocrab");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "autocrab");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/autocrab-path/case-linuxbrew");
