...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__autocrab_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "autocrab-root");
...
describe("resolveAutoCrabPackageRoot", () => {
  let resolveAutoCrabPackageRoot: typeof import("./autocrab-root.js").resolveAutoCrabPackageRoot;
  let resolveAutoCrabPackageRootSync: typeof import("./autocrab-root.js").resolveAutoCrabPackageRootSync;

  beforeAll(async () => {
    ({ resolveAutoCrabPackageRoot, resolveAutoCrabPackageRootSync } =
      await import("./autocrab-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "autocrab");
    const pkgRoot = path.join(project, "node_modules", "autocrab");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "autocrab" }));

    expect(resolveAutoCrabPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "autocrab");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "autocrab.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "autocrab" }));

    expect(resolveAutoCrabPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveAutoCrabPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-autocrab package roots", async () => {
    const pkgRoot = fx("not-autocrab");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-autocrab" }));

    expect(resolveAutoCrabPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveAutoCrabPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveAutoCrabPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
