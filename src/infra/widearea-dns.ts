...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AUTOCRAB_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*autocrab-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "autocrab";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "autocrab");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "autocrab-gw");
  ...
  records.push(`_autocrab-gw._tcp IN PTR ${instanceLabel}._autocrab-gw._tcp`);
  records.push(`${instanceLabel}._autocrab-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._autocrab-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; autocrab-content-hash: ${contentHash}\n${contentBody}`;
}
