...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set AUTOCRAB_GATEWAY_TOKEN/AUTOCRAB_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "autocrab-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.AUTOCRAB_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.AUTOCRAB_GATEWAY_PASSWORD);
