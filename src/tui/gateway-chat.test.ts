...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["AUTOCRAB_GATEWAY_TOKEN", "AUTOCRAB_GATEWAY_PASSWORD"]);
  ...
  it.each([
    ...
    const result = await withEnvAsync({ AUTOCRAB_GATEWAY_TOKEN: "env-token" }, async () => {
  ...
  it("uses AUTOCRAB_GATEWAY_TOKEN for local mode", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    await withEnvAsync({ AUTOCRAB_GATEWAY_TOKEN: "env-token" }, async () => {
      const result = await resolveGatewayConnection({});
  ...
  it("prefers AUTOCRAB_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    await withEnvAsync({ AUTOCRAB_GATEWAY_PASSWORD: "env-pass" }, async () => {
      const result = await resolveGatewayConnection({});
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-tui-file-secret-"));
  ...
  it("resolves only token SecretRef when gateway.auth.mode is token", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-tui-mode-token-"));
  ...
  it("resolves only password SecretRef when gateway.auth.mode is password", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-tui-mode-password-"));
    const tokenMarker = path.join(tempDir, "token-provider-ran");
    const passwordMarker = path.join(tempDir, "password-provider-ran");
    const tokenExecProgram = [
      "const fs=require('node:fs');",
