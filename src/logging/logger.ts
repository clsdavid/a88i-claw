...
export const DEFAULT_LOG_DIR = resolvePreferredAutoCrabTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "autocrab.log"); // legacy single-file path

const LOG_PREFIX = "autocrab";
...
function canUseSilentVitestFileLogFastPath(envLevel: LogLevel | undefined): boolean {
  return (
    ...
    process.env.AUTOCRAB_TEST_FILE_LOG !== "1" &&
    !envLevel &&
...
function resolveSettings(): ResolvedSettings {
  ...
  let cfg: AutoCrabConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg && !shouldSkipLoadConfigFallback()) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => AutoCrabConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.AUTOCRAB_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[autocrab] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
