...
let originalAutoCrabLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.AUTOCRAB_TEST_FILE_LOG;
  originalAutoCrabLogLevel = process.env.AUTOCRAB_LOG_LEVEL;
  delete process.env.AUTOCRAB_TEST_FILE_LOG;
  delete process.env.AUTOCRAB_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.AUTOCRAB_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalAutoCrabLogLevel === undefined) {
    delete process.env.AUTOCRAB_LOG_LEVEL;
  } else {
    process.env.AUTOCRAB_LOG_LEVEL = originalAutoCrabLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
