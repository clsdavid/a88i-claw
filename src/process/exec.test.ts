...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        AUTOCRAB_BASE_ENV: "base",
        AUTOCRAB_TO_REMOVE: undefined,
      },
      env: {
        AUTOCRAB_TEST_ENV: "ok",
      },
    });

    expect(resolved.AUTOCRAB_BASE_ENV).toBe("base");
    expect(resolved.AUTOCRAB_TEST_ENV).toBe("ok");
    expect(resolved.AUTOCRAB_TO_REMOVE).toBeUndefined();
  });
