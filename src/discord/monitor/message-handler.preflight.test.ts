...
describe("preflightDiscordMessage", () => {
  ...
  it("drops bound-thread bot system messages to prevent ACP self-loop", async () => {
    ...
    const result = await preflightDiscordMessage({
      ...
      } as import("../../config/config.js").AutoCrabConfig,
      discordConfig: {
        allowBots: true,
      } as NonNullable<import("../../config/config.js").AutoCrabConfig["channels"]>["discord"],
      accountId: "default",
      token: "token",
      runtime: {} as import("../../runtime.js").RuntimeEnv,
      botUserId: "autocrab-bot",
  ...
  it("allows bot messages with explicit mention when allowBots=mentions", async () => {
    ...
    const message = {
      ...
      content: "hi <@autocrab-bot>",
      timestamp: new Date().toISOString(),
      channelId,
      attachments: [],
      mentionedUsers: [{ id: "autocrab-bot" }],
      mentionedRoles: [],
      mentionedEveryone: false,
      author: {
        id: "relay-bot-1",
  ...
  it("drops guild messages that mention another user when ignoreOtherMentions=true", async () => {
    ...
    const result = await preflightDiscordMessage({
      ...
      discordConfig: {} as NonNullable<
        import("../../config/config.js").AutoCrabConfig["channels"]
      >["discord"],
      accountId: "default",
      token: "token",
      runtime: {} as import("../../runtime.js").RuntimeEnv,
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey autocrab");
