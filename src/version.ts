...
declare const __AUTOCRAB_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "autocrab";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["AUTOCRAB_VERSION"],
      runtimeVersion,
      env["AUTOCRAB_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current AutoCrab version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __AUTOCRAB_VERSION__ === "string" ? __AUTOCRAB_VERSION__ : undefined,
  bundledVersion: process.env.AUTOCRAB_BUNDLED_VERSION,
});
