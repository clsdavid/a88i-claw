...
vi.mock("./chrome.js", () => ({
  resolveAutoCrabUserDataDir: vi.fn(() => "/tmp/autocrab-test/autocrab/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveAutoCrabUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "autocrab-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveAutoCrabUserDataDir).mockReturnValue(userDataDir);
