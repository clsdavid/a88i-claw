...
export function resolveAutoCrabUserDataDir(profileName = DEFAULT_AUTOCRAB_BROWSER_PROFILE_NAME) {
...
export async function launchAutoCrabChrome(
  ...
  const userDataDir = resolveAutoCrabUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_AUTOCRAB_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateAutoCrabProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 autocrab browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`autocrab browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`autocrab browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 autocrab browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopAutoCrabChrome(
