...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("autocrab");
    expect(profile?.driver).toBe("autocrab");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const autocrab = resolveProfile(resolved, "autocrab");
    expect(autocrab?.driver).toBe("autocrab");
    expect(autocrab?.cdpPort).toBe(18800);
    expect(autocrab?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from AUTOCRAB_GATEWAY_PORT when unset", () => {
    withEnv({ AUTOCRAB_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const autocrab = resolveProfile(resolved, "autocrab");
      expect(autocrab?.cdpPort).toBe(19012);
      expect(autocrab?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ AUTOCRAB_GATEWAY_PORT: undefined }, () => {
      ...
      const autocrab = resolveProfile(resolved, "autocrab");
      expect(autocrab?.cdpPort).toBe(19022);
      expect(autocrab?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(autocrab?.cdpPort).toBe(19000);
    expect(autocrab?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "autocrab");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        autocrab: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("autocrab");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to autocrab profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps autocrab default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("autocrab");
    ...
    it("keeps autocrab default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("autocrab");
    ...
    it("keeps autocrab default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
