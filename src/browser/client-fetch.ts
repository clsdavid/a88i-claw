...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-autocrab-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-autocrab-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-autocrab-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the AutoCrab gateway (AutoCrab.app menubar, or \`${formatCliCommand("autocrab gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the AutoCrab browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the AutoCrab browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
