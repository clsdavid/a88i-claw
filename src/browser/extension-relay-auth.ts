...
const RELAY_TOKEN_CONTEXT = "autocrab-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const AUTOCRAB_RELAY_BROWSER = "AutoCrab/extension-relay";
...
async function resolveGatewayAuthToken(): Promise<string | null> {
  const envToken =
    process.env.AUTOCRAB_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
    ...
    if (tokenRef) {
      ...
      throw new SecretRefUnavailableError(
        `extension relay requires a resolved gateway token, but gateway.auth.token SecretRef is unavailable (${refLabel}). Set AUTOCRAB_GATEWAY_TOKEN or resolve your secret provider.`,
      );
    }
...
export async function resolveRelayAcceptedTokensForPort(port: number): Promise<string[]> {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or AUTOCRAB_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedAutoCrabRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === AUTOCRAB_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
