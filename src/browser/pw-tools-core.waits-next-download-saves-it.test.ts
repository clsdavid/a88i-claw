...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredAutoCrabTmpDir: vi.fn(() => "/tmp/autocrab"),
}));
vi.mock("../infra/tmp-autocrab-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredAutoCrabTmpDir.mockReturnValue("/tmp/autocrab");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".autocrab-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredAutoCrabTmpDir.mockReturnValue("/tmp/autocrab-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "autocrab-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "autocrab-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredAutoCrabTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "autocrab-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "autocrab-preferred", "downloads")}${path.sep}`),
    );
  });
