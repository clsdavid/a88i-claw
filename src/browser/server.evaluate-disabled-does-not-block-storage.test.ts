  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          autocrab: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.AUTOCRAB_GATEWAY_PORT;
    process.env.AUTOCRAB_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.AUTOCRAB_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.AUTOCRAB_GATEWAY_PASSWORD;
