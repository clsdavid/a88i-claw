...
import {
  probeAuthenticatedAutoCrabRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedAutoCrabRelay({
    baseUrl,
    relayAuthHeader: "x-autocrab-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated autocrab relay probe responses", async () => {
