...
const RELAY_AUTH_HEADER = "x-autocrab-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/AutoCrab-Extension-Relay",
            revision: "0",
            userAgent: "AutoCrab-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "AutoCrab/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedAutoCrabRelay({
