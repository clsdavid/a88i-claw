import type { BrowserConfig, BrowserProfileConfig, AutoCrabConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_AUTOCRAB_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_AUTOCRAB_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "autocrab" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_AUTOCRAB_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_AUTOCRAB_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an AutoCrab browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for autocrab/autocrab CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AutoCrabConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_AUTOCRAB_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_AUTOCRAB_BROWSER_PROFILE_NAME]
        ? DEFAULT_AUTOCRAB_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "autocrab";
