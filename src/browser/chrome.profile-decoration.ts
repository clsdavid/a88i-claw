...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".autocrab-profile-decorated");
}
 ...
 */
export function decorateAutoCrabProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_AUTOCRAB_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_AUTOCRAB_BROWSER_COLOR).toUpperCase();
