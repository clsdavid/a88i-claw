...
function createReplyConfig(home: string, streamMode?: "block"): AutoCrabConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("autocrab-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("AUTOCRAB_TEST_FAST", "1");
