...
export function isCommandEnabled(cfg: AutoCrabConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AutoCrabConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AutoCrabConfig),
...
export function getCommandDetection(_cfg?: AutoCrabConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AutoCrabConfig) {
