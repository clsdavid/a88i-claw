...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop autocrab!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@autocrab_bot", { botUsername: "autocrab_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@autocrab_bot", { botUsername: "autocrab_bot" })).toBe(true);
