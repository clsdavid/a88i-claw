...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-commands-"));
...
function buildParams(commandBody: string, cfg: AutoCrabConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/autocrab-session-store.json" },
    } as AutoCrabConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/autocrab-agent-compact";
...
describe("handleCommands /allowlist", () => {
  ...
  it("removes DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    ...
    for (const testCase of cases) {
      ...
      const written = writeConfigFileMock.mock.calls.at(-1)?.[0] as AutoCrabConfig;
...
describe("handleCommands ACP-bound /new and /reset", () => {
  ...
  const buildDiscordBoundConfig = (): AutoCrabConfig =>
    ({
      commands: { text: true },
      bindings: [
        {
