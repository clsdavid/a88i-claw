...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    autocrabHome: process.env.AUTOCRAB_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("AUTOCRAB_HOME", snapshot.autocrabHome);
  restoreKey("AUTOCRAB_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.AUTOCRAB_STATE_DIR = join(home, ".autocrab");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "autocrab-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".autocrab", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): AutoCrabConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "autocrab"),
