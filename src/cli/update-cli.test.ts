...
import type { AutoCrabConfig, ConfigFileSnapshot } from "../config/types.autocrab.js";
...
vi.mock("../infra/autocrab-root.js", () => ({
  resolveAutoCrabPackageRoot: vi.fn(),
}));
...
const { resolveAutoCrabPackageRoot } = await import("../infra/autocrab-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/autocrab-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as AutoCrabConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/autocrab-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveAutoCrabPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("autocrab-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveAutoCrabPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("autocrab");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/autocrab-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "autocrab-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("AutoCrab update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as AutoCrabConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveAutoCrabPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("autocrab-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ AUTOCRAB_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.AUTOCRAB_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("autocrab-update-wizard");
    await withEnvAsync({ AUTOCRAB_GIT_DIR: tempDir }, async () => {
      setTty(true);
