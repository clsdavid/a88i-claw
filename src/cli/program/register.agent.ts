...
${formatHelpExamples([
  ['autocrab agent --to +15555550123 --message "status update"', "Start a new session."],
  ['autocrab agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'autocrab agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'autocrab agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['autocrab agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.autocrab.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.autocrab.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['autocrab agents set-identity --agent main --name "AutoCrab" --emoji "🦞"', "Set name + emoji."],
  ["autocrab agents set-identity --agent main --avatar avatars/autocrab.png", "Set avatar path."],
  [
    "autocrab agents set-identity --workspace ~/.autocrab/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "autocrab agents set-identity --identity-file ~/.autocrab/workspace/IDENTITY.md --agent main",
