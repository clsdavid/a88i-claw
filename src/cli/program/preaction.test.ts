...
beforeEach(() => {
  ...
  originalHideBanner = process.env.AUTOCRAB_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.AUTOCRAB_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.AUTOCRAB_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("autocrab");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "autocrab", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("autocrab-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "autocrab", "message", "send"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "autocrab", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.AUTOCRAB_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "autocrab", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "autocrab", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "autocrab", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "autocrab", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "autocrab", "--profile", "work", "config", "validate"],
    });
