    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["autocrab nodes status", "List known nodes with live status."],
          ["autocrab nodes pairing pending", "Show pending node pairing requests."],
          ['autocrab nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["autocrab nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.autocrab.ai/cli/nodes")}\n`,
    );
