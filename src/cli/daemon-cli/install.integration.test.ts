...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("autocrab-daemon-install-int-");
    configPath = path.join(tempHome, "autocrab.json");
    process.env.HOME = tempHome;
    process.env.AUTOCRAB_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists and persists the same token used for install env", async () => {
    ...
    expect(installEnv?.AUTOCRAB_GATEWAY_TOKEN).toBe(persistedToken);
  });
});
