...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@autocrab/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@autocrab/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@autocrab/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@autocrab/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @autocrab/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@autocrab/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@autocrab/plugin-alpha@1.2.3",
        installPath: "/tmp/autocrab/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@autocrab/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @autocrab/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@autocrab/plugin-alpha@latest",
      true,
      "/tmp/autocrab/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@autocrab/plugin-alpha@latest",
      installPath: "/tmp/autocrab/extensions/alpha",
      version: "1.2.3",
