...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubEnv("AUTOCRAB_GATEWAY_TOKEN", "");
    vi.stubEnv("CLAWDBOT_GATEWAY_TOKEN", "");
    vi.stubEnv("AUTOCRAB_GATEWAY_PASSWORD", "");
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("autocrab devices approve <requestId>");
  });
  ...
  it("uses AUTOCRAB_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("AUTOCRAB_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue({
      secrets: {
        providers: {
          default: { source: "env" },
