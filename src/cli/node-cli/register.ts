    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "autocrab node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["autocrab node status", "Check node host service status."],
          ["autocrab node install", "Install the node host service."],
          ["autocrab node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.autocrab.ai/cli/node")}\n`,
    );
