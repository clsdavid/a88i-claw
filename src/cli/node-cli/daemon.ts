...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("autocrab node install"), formatCliCommand("autocrab node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("autocrab node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    AUTOCRAB_LOG_PREFIX: baseEnv.AUTOCRAB_LOG_PREFIX ?? "node",
