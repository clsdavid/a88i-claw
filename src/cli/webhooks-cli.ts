...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.autocrab.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + AutoCrab hooks")
    ...
    .option("--hook-url <url>", "AutoCrab hook URL")
    .option("--hook-token <token>", "AutoCrab hook token")
