...
const SANDBOX_EXAMPLES = {
  main: [
    ["autocrab sandbox list", "List all sandbox containers."],
    ["autocrab sandbox list --browser", "List only browser containers."],
    ["autocrab sandbox recreate --all", "Recreate all containers."],
    ["autocrab sandbox recreate --session main", "Recreate a specific session."],
    ["autocrab sandbox recreate --agent mybot", "Recreate agent containers."],
    ["autocrab sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["autocrab sandbox list", "List all sandbox containers."],
    ["autocrab sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["autocrab sandbox recreate --all", "Recreate all containers."],
    ["autocrab sandbox recreate --session main", "Recreate a specific session."],
    ["autocrab sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["autocrab sandbox recreate --browser --all", "Recreate only browser containers."],
    ["autocrab sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["autocrab sandbox explain", "Show effective sandbox config."],
    ["autocrab sandbox explain --session agent:main:main", "Explain a specific session."],
    ["autocrab sandbox explain --agent work", "Explain an agent sandbox."],
    ["autocrab sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.autocrab.ai/cli/sandbox")}\n`,
    )
    .action(() => {
