...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "autocrab-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "autocrab-bundled-skills-test-"));
    process.env.AUTOCRAB_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
