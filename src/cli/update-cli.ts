...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update AutoCrab and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["autocrab update", "Update a source checkout (git)"],
        ["autocrab update --channel beta", "Switch to beta channel (git + npm)"],
        ["autocrab update --channel dev", "Switch to dev channel (git + npm)"],
        ["autocrab update --tag beta", "One-off update to a dist-tag or version"],
        ["autocrab update --dry-run", "Preview actions without changing anything"],
        ["autocrab update --no-restart", "Update without restarting the service"],
        ["autocrab update --json", "Output result as JSON"],
        ["autocrab update --yes", "Non-interactive (accept downgrade prompts)"],
        ["autocrab update wizard", "Interactive update wizard"],
        ["autocrab --update", "Shorthand for autocrab update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run autocrab update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.autocrab.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["autocrab update status", "Show channel + version status."],
          ["autocrab update status --json", "JSON output."],
          ["autocrab update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.autocrab.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
