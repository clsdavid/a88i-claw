...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("autocrab-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.AUTOCRAB_STATE_DIR).toBe(path.join(tempHome.home, ".autocrab"));
    ...
    expect(process.env.AUTOCRAB_STATE_DIR).toBe(previousStateDir);
