...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `autocrab-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      AUTOCRAB_LAUNCHD_LABEL: `ai.autocrab.launchd-int-${testId}`,
      AUTOCRAB_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
