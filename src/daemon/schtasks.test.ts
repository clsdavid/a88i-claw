...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when AUTOCRAB_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".autocrab", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when AUTOCRAB_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", AUTOCRAB_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".autocrab-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers AUTOCRAB_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        AUTOCRAB_PROFILE: "rescue",
        AUTOCRAB_STATE_DIR: "C:\\State\\autocrab",
      },
      expected: path.join("C:\\State\\autocrab", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", AUTOCRAB_PROFILE: "default" },
      expected: path.join("/home/test", ".autocrab", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem AutoCrab Gateway",
          "cd /d C:\\Projects\\autocrab",
          "set NODE_ENV=production",
          "set AUTOCRAB_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\autocrab",
          environment: {
            NODE_ENV: "production",
            AUTOCRAB_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\autocrab\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\autocrab\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\AutoCrab Share\\node.exe" "\\\\fileserver\\AutoCrab Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\AutoCrab Share\\node.exe",
            "\\\\fileserver\\AutoCrab Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from AUTOCRAB_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ AUTOCRAB_STATE_DIR: path.join(tmpDir, "custom-state") }),
