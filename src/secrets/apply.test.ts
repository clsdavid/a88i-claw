...
function buildFixturePaths(rootDir: string) {
  const stateDir = path.join(rootDir, ".autocrab");
  return {
    rootDir,
    stateDir,
...
async function createApplyFixture(): Promise<ApplyFixture> {
  const paths = buildFixturePaths(
    await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-secrets-apply-")),
  );
  ...
  return {
    ...paths,
    env: {
      AUTOCRAB_STATE_DIR: paths.stateDir,
      AUTOCRAB_CONFIG_PATH: paths.configPath,
