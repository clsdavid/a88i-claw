...
describe("Dockerfile", () => {
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG AUTOCRAB_INSTALL_BROWSER");
  ...
  it("Docker GPG fingerprint awk uses correct quoting for AUTOCRAB_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
