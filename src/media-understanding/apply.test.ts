...
const TEMP_MEDIA_PREFIX = "autocrab-media-";
...
function createGroqAudioConfig(): AutoCrabConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): AutoCrabConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): AutoCrabConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: AutoCrabConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredAutoCrabTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        AUTOCRAB_AGENT_DIR: isolatedAgentDir,
