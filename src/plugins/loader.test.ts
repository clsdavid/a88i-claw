...
import { __testing, loadAutoCrabPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "autocrab-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.AUTOCRAB_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "autocrab.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.AUTOCRAB_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadAutoCrabPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          autocrab: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.AUTOCRAB_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.AUTOCRAB_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadAutoCrabPlugins>) {
...
function useNoBundledPlugins() {
  process.env.AUTOCRAB_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadAutoCrabPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.AUTOCRAB_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.AUTOCRAB_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadAutoCrabPlugins", () => {
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadAutoCrabPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadAutoCrabPlugins(options);
  ...
  it("skips importing bundled memory plugins that are disabled by memory slot", () => {
    ...
    fs.writeFileSync(
      path.join(memoryADir, "autocrab.plugin.json"),
      JSON.stringify(
        {
          id: "memory-a",
          kind: "memory",
    ...
    fs.writeFileSync(
      path.join(memoryBDir, "autocrab.plugin.json"),
      JSON.stringify(
        {
          id: "memory-b",
          kind: "memory",
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ AUTOCRAB_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("autocrab/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
