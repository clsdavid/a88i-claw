...
import { discoverAutoCrabPlugins } from "./discovery.js";
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const entry of pluginSdkScopedAliasEntries) {
    ...
    if (resolved) {
      aliasMap[`autocrab/plugin-sdk/${entry.subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AutoCrabPluginDefinition;
  register?: AutoCrabPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AutoCrabPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AutoCrabPluginDefinition;
...
export function loadAutoCrabPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAutoCrabPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "autocrab/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AutoCrabPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as AutoCrabPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
