...
function createSinglePluginWithEmptySlotsConfig(): AutoCrabConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): AutoCrabConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "autocrab-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-autocrab-extensions/my-plugin",
      },
      extensionsDir,
    });
