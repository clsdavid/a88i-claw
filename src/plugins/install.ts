...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing autocrab.extensions; update the plugin package to include autocrab.extensions (for example ["./dist/index.js"]). See https://docs.autocrab.ai/help/troubleshooting#plugin-install-fails-with-missing-autocrab-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_AUTOCRAB_EXTENSIONS: "missing_autocrab_extensions",
  EMPTY_AUTOCRAB_EXTENSIONS: "empty_autocrab_extensions",
...
function ensureAutoCrabExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_AUTOCRAB_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json autocrab.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_AUTOCRAB_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureAutoCrabExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from autocrab.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-autocrab"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "autocrab security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "autocrab security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "autocrab-npm-pack-",
    spec,
    timeoutMs,
