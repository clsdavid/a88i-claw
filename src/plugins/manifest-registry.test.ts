...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `autocrab-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "autocrab.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "autocrab.plugin.json");
  const linkedManifest = path.join(rootDir, "autocrab.plugin.json");
