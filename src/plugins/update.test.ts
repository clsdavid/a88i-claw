...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-autocrab",
      targetDir: "/tmp/opik-autocrab",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-autocrab": {
              source: "npm",
              spec: "@opik/opik-autocrab",
              integrity: "sha512-old",
              installPath: "/tmp/opik-autocrab",
            },
          },
        },
      },
      pluginIds: ["opik-autocrab"],
      dryRun: true,
    });
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @autocrab/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @autocrab/missing.",
      },
    ]);
  });
