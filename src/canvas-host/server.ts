...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.autocrabCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.autocrabCanvasA2UIAction &&
        typeof window.autocrabCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.autocrabSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("autocrab:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android AutoCrab node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.autocrabSendUserAction === "function"
        ? window.autocrabSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.AUTOCRAB_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.AUTOCRAB_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>AutoCrab Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
