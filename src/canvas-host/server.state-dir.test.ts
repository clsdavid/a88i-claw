...
describe("canvas host state dir defaults", () => {
  it("uses AUTOCRAB_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("autocrab-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("AutoCrab Canvas");
      } finally {
        await handler.close();
      }
    });
