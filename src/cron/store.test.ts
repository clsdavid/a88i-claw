...
const { makeStorePath } = createCronStoreHarness({ prefix: "autocrab-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses AUTOCRAB_HOME for tilde expansion", () => {
    vi.stubEnv("AUTOCRAB_HOME", "/srv/autocrab-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/autocrab-home", "cron", "jobs.json"));
  });
});
