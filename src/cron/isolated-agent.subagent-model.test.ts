...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "autocrab-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".autocrab", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<AutoCrabConfig> = {},
): AutoCrabConfig {
  const base: AutoCrabConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<AutoCrabConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<AutoCrabConfig>,
