...
const defaultTools = createAutoCrabCodingTools({ senderIsOwner: true });
...
describe("createAutoCrabCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: AutoCrabConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createAutoCrabCodingTools({
    ...
    const anthropicTools = createAutoCrabCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: AutoCrabConfig = {
      tools: {
        exec: {
    ...
    const allowed = createAutoCrabCodingTools({
    ...
    const denied = createAutoCrabCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createAutoCrabCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
