...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureAutoCrabModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): AutoCrabConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): AutoCrabConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): AutoCrabConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-workspace-"));
    const now = Date.now();
