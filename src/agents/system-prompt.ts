...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; AutoCrab handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "When diagnosing issues, run `autocrab status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside AutoCrab.";
  }

  const lines = [
    "You are a personal assistant running inside AutoCrab.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control AutoCrab's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
    ...
    "",
    ...safetySection,
    "## AutoCrab CLI Quick Reference",
    "AutoCrab is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- autocrab gateway status",
    "- autocrab gateway start",
    "- autocrab gateway stop",
    "- autocrab gateway restart",
    "If unsure, ask the user to run `autocrab help` (or `autocrab gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## AutoCrab Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, AutoCrab pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by AutoCrab and included below in Project Context.",
    "",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'AutoCrab treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
