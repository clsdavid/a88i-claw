...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses AUTOCRAB_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      AUTOCRAB_HOME: "/srv/autocrab-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/autocrab-home"), ".autocrab", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".autocrab", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("autocrab-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-workspace-hardlink-"));
    try {
