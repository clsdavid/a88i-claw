...
const originalBundledDir = process.env.AUTOCRAB_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("autocrab-bundled-");
  const workspaceDir = await createTempDir("autocrab-workspace-");
  const pluginRoot = path.join(bundledPluginsDir, "diffs");

  await fs.mkdir(path.join(pluginRoot, "skills", "diffs"), { recursive: true });
  await fs.writeFile(
    path.join(pluginRoot, "autocrab.plugin.json"),
    JSON.stringify(
      {
        id: "diffs",
        skills: ["./skills"],
...
afterEach(async () => {
  process.env.AUTOCRAB_BUNDLED_PLUGINS_DIR = originalBundledDir;
...
describe("resolveEmbeddedRunSkillEntries (integration)", () => {
  it("loads bundled diffs skill when explicitly enabled in config", async () => {
    ...
    process.env.AUTOCRAB_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
    clearPluginManifestRegistryCache();
