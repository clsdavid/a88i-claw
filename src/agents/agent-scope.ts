...
type AgentEntry = NonNullable<NonNullable<AutoCrabConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: AutoCrabConfig): AgentEntry[] {
...
export function listAgentIds(cfg: AutoCrabConfig): string[] {
...
export function resolveDefaultAgentId(cfg: AutoCrabConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: AutoCrabConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: AutoCrabConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: AutoCrabConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: AutoCrabConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
