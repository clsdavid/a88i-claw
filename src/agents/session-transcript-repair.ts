...
function redactSessionsSpawnAttachmentsArgs(value: unknown): unknown {
  ...
  const next = raw.map((item) => {
    ...
    return { ...rest, content: "__AUTOCRAB_REDACTED__" };
  });
...
function makeMissingToolResult(params: {
  ...
  return {
    ...
    content: [
      {
        type: "text",
        text: "[autocrab] missing tool result in session history; inserted synthetic error result for transcript repair.",
      },
    ],
    isError: true,
    timestamp: Date.now(),
...
export function repairToolUseResultPairing(messages: AgentMessage[]): ToolUseRepairReport {
  ...
  for (let i = 0; i < messages.length; i += 1) {
    ...
    // See: https://github.com/autocrab/autocrab/issues/4597
