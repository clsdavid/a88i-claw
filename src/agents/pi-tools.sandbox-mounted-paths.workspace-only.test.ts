...
type CodingToolsInput = NonNullable<Parameters<typeof createAutoCrabCodingTools>[0]>;
...
function resolveApplyPatchTool(
  params: Pick<CodingToolsInput, "sandbox" | "workspaceDir"> & { config: AutoCrabConfig },
): ToolWithExecute {
...
describe("tools.fs.workspaceOnly", () => {
  it("defaults to allowing sandbox mounts outside the workspace root", async () => {
    await withUnsafeMountedSandboxHarness(async ({ sandboxRoot, agentRoot, sandbox }) => {
      ...
      const tools = createAutoCrabCodingTools({ sandbox, workspaceDir: sandboxRoot });
  ...
  it("rejects sandbox mounts outside the workspace root when enabled", async () => {
    await withUnsafeMountedSandboxHarness(async ({ sandboxRoot, agentRoot, sandbox }) => {
      ...
      const cfg = { tools: { fs: { workspaceOnly: true } } } as unknown as AutoCrabConfig;
      const tools = createAutoCrabCodingTools({ sandbox, workspaceDir: sandboxRoot, config: cfg });
