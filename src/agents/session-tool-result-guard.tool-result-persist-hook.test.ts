...
import { loadAutoCrabPlugins } from "../plugins/loader.js";
...
describe("tool_result_persist hook", () => {
  ...
  it("loads tool_result_persist hooks without breaking persistence", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "autocrab-toolpersist-"));
...
describe("before_message_write hook", () => {
  it("continues persistence when a before_message_write hook throws", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "autocrab-before-write-"));
    process.env.AUTOCRAB_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
