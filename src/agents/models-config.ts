...
type ModelsConfig = NonNullable<AutoCrabConfig["models"]>;
...
export async function ensureAutoCrabModelsJson(
  config?: AutoCrabConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveAutoCrabAgentDir();
