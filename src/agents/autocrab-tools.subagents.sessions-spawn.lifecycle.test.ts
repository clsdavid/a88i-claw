...
} from "./autocrab-tools.subagents.sessions-spawn.test-harness.js";
import { resetSubagentRegistryForTests } from "./subagent-registry.js";

const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.AUTOCRAB_TEST_FAST;
...
describe("autocrab-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    if (fastModeEnv.previous === undefined) {
      delete process.env.AUTOCRAB_TEST_FAST;
      return;
    }
    process.env.AUTOCRAB_TEST_FAST = fastModeEnv.previous;
  });
