...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "autocrab.sandbox=1");
  args.push("--label", `autocrab.sessionKey=${params.scopeKey}`);
  args.push("--label", `autocrab.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `autocrab.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "autocrab.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`autocrab sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`autocrab sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("autocrab sandbox recreate --all");
}
