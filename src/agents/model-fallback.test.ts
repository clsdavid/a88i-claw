...
function makeFallbacksOnlyCfg(): AutoCrabConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): AutoCrabConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function withTempAuthStore<T>(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-auth-"));
...
// https://github.com/autocrab/autocrab/issues/23440
const INSUFFICIENT_QUOTA_PAYLOAD =
  '{"type":"error","error":{"type":"insufficient_quota","message":"Your account has insufficient quota balance to run this request."}}';
// Internal AutoCrab compatibility marker, not a provider API contract.
...
describe("runWithModelFallback", () => {
  ...
  describe("fallback behavior with provider cooldowns", () => {
    async function makeAuthStoreWithCooldown(
      ...
      const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-test-"));
      const now = Date.now();
      const store: AuthProfileStore = {
        version: AUTH_STORE_VERSION,
        profiles: {
