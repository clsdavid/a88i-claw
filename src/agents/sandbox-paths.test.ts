...
async function withOutsideHardlinkInAutoCrabTmp<T>(
  params: {
    autoCrabTmpDir: string;
  ...
  const hardlinkPath = path.join(params.autoCrabTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.autoCrabTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.autoCrabTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.autoCrabTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const autoCrabTmpDir = resolvePreferredAutoCrabTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred AutoCrab tmp root",
      media: path.join(autoCrabTmpDir, "image.png"),
      expected: path.join(autoCrabTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred AutoCrab tmp root",
      media: pathToFileURL(path.join(autoCrabTmpDir, "photo.png")).href,
      expected: path.join(autoCrabTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred AutoCrab tmp root",
      media: path.join(autoCrabTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(autoCrabTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(autoCrabTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside autocrab tmp root",
      media: path.join(os.tmpdir(), "outside-autocrab", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked AutoCrab tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(autoCrabTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(autoCrabTmpDir, { recursive: true });
      const symlinkPath = path.join(autoCrabTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked AutoCrab tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAutoCrabTmp(
      {
        autoCrabTmpDir,
  ...
  it("rejects symlinked AutoCrab tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAutoCrabTmp(
