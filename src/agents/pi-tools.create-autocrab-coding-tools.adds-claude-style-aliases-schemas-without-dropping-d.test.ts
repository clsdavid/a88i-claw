...
describe("createAutoCrabCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-read-"));
    try {
      ...
      const contents = "Hello from autocrab read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createAutoCrabCodingTools({ sandbox });
