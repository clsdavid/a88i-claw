...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["AUTOCRAB_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-subagent-"));
  ...
  it("uses isolated temp state when AUTOCRAB_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "autocrab-test-state"));
  });
});
