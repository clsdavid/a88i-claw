...
const GOOGLE_LIVE_SINGLE_KEY = "AUTOCRAB_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "AUTOCRAB_LIVE_ANTHROPIC_KEY",
    listVar: "AUTOCRAB_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "AUTOCRAB_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `AUTOCRAB_LIVE_${base}_KEY`;
