...
test("exec sets AUTOCRAB_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-autocrab-shell", {
    command: "node -e \"process.stdout.write(process.env.AUTOCRAB_SHELL || '')\"",
    pty: true,
  });
