...
type CoreToolDefinition = {
  ...
  includeInAutoCrabGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const autocrabTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInAutoCrabGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:autocrab": autocrabTools,
