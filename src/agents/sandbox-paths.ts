...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const autoCrabTmpDir = path.resolve(resolvePreferredAutoCrabTmpDir());
  if (!isPathInside(autoCrabTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: autoCrabTmpDir });
  return resolved;
}
