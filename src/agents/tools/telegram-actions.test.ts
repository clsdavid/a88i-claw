...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): AutoCrabConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as AutoCrabConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): AutoCrabConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as AutoCrabConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: AutoCrabConfig, accountId = "media") {
    await handleTelegramAction(
