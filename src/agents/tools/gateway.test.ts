...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    autocrab: process.env.AUTOCRAB_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.autocrab === undefined) {
      delete process.env.AUTOCRAB_GATEWAY_TOKEN;
    } else {
      process.env.AUTOCRAB_GATEWAY_TOKEN = envSnapshot.autocrab;
    }
  ...
  it("uses AUTOCRAB_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.AUTOCRAB_GATEWAY_TOKEN = "local-env-token";
