...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: AutoCrabConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/autocrab" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: AutoCrabConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: AutoCrabConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: AutoCrabConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: AutoCrabConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: AutoCrabConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: AutoCrabConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: AutoCrabConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/autocrab");
  });

  it("uses AUTOCRAB_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "autocrab-home");
    vi.stubEnv("AUTOCRAB_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as AutoCrabConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".autocrab", "workspace"));
  });

  it("uses AUTOCRAB_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "autocrab-home");
    vi.stubEnv("AUTOCRAB_HOME", home);
    // Clear state dir so it falls back to AUTOCRAB_HOME
    vi.stubEnv("AUTOCRAB_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as AutoCrabConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".autocrab", "agents", "main", "agent"));
  });
});
