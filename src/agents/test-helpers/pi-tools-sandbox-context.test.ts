...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("autocrab-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("autocrab-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("autocrab-sbx-");
