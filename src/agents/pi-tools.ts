...
function applyModelProviderToolPolicy(
  ...
  // xAI/Grok providers expose a native web_search tool; sending AutoCrab's
...
function resolveExecConfig(params: { cfg?: AutoCrabConfig; agentId?: string }) {
...
export function createAutoCrabCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createAutoCrabReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
