...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.AUTOCRAB_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".autocrab", `workspace-${profile}`);
  }
  return path.join(home, ".autocrab", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".autocrab";
