...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "autocrab-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.AUTOCRAB_BUNDLED_SKILLS_DIR = bundledDir;
