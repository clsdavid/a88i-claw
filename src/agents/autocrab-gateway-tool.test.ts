...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createAutoCrabTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-test-"));

    try {
      await withEnvAsync(
        { AUTOCRAB_STATE_DIR: stateDir, AUTOCRAB_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: autocrab --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/autocrab" } }\n}\n';
