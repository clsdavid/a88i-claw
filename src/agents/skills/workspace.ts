...
function resolveSkillsLimits(config?: AutoCrabConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "autocrab-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "autocrab-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveAutoCrabMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: AutoCrabConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`autocrab skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
