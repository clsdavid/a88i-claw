    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "autocrab.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "autocrab.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "autocrab.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("autocrab-");
  const acpxRoot = await tempDirs.make("autocrab-acpx-plugin-");
  const helperRoot = await tempDirs.make("autocrab-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("autocrab-");
  const pluginRoot = await tempDirs.make("autocrab-plugin-");
  const outsideDir = await tempDirs.make("autocrab-outside-");
