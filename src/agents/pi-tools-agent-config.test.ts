...
import { createAutoCrabCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: AutoCrabConfig) {
    return createAutoCrabCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<AutoCrabConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<AutoCrabConfig["agents"]>["list"]>[number]["tools"];
  }): AutoCrabConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createAutoCrabCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createAutoCrabCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createAutoCrabCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createAutoCrabCodingTools({
      config: cfg,
    ...
    const bobTools = createAutoCrabCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createAutoCrabCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createAutoCrabCodingTools({
      config: cfg,
