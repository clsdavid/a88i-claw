 ...
 * A common mistake — caused by the similarity with the `autocrab.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (autocrab.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.AUTOCRAB_AUTH_STORE_READONLY === "1";
