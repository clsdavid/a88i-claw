...
This directory contains hooks that ship with AutoCrab. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.autocrab/autocrab.json`:
...
2. Restart gateway: `pkill -9 -f 'autocrab.*gateway' && pnpm autocrab gateway`
3. Enable the hook: `autocrab hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.autocrab.ai/automation/hooks
