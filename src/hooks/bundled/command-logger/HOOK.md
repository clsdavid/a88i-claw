...
homepage: https://docs.autocrab.ai/automation/hooks#command-logger
metadata:
  {
    "autocrab":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.autocrab/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.autocrab/logs/commands.log ~/.autocrab/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/autocrab`:
   ```
   /home/username/.autocrab/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
