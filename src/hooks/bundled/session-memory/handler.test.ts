...
beforeAll(async () => {
  ...
  suiteWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-session-memory-"));
});
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => AutoCrabConfig;
  action?: "new" | "reset";
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): AutoCrabConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
