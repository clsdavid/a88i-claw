...
`;
    ...
    expect(parsed.autocrab.emoji).toBe("💾");
    expect(parsed.autocrab.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.autocrab.emoji).toBe("📝");
    expect(parsed.autocrab.events).toEqual(["command"]);
    expect(parsed.autocrab.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.autocrab.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"autocrab": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"autocrab": {"events": ["test"]}}');
  });
...
describe("resolveAutoCrabMetadata", () => {
  it("extracts autocrab metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAutoCrabMetadata(frontmatter);
  ...
  it("returns undefined when autocrab key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        autocrab: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with AutoCrab" },
            { id: "npm", kind: "npm", package: "@autocrab/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@autocrab/hook");
  });
...
`;
    ...
    const autocrab = resolveAutoCrabMetadata(frontmatter);
    expect(autocrab).toBeDefined();
    expect(autocrab?.emoji).toBe("💾");
    expect(autocrab?.events).toEqual(["command:new", "command:reset"]);
    expect(autocrab?.requires?.config).toEqual(["workspace.dir"]);
    expect(autocrab?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const autocrab = resolveAutoCrabMetadata(frontmatter);
    expect(autocrab?.emoji).toBe("disk");
    expect(autocrab?.events).toEqual(["command:new"]);
  });
});
