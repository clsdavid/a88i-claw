...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.AUTOCRAB_SKIP_CHANNELS;
    prevSkipGmail = process.env.AUTOCRAB_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.AUTOCRAB_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.AUTOCRAB_SKIP_CHANNELS = "0";
    delete process.env.AUTOCRAB_SKIP_GMAIL_WATCHER;
    delete process.env.AUTOCRAB_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.AUTOCRAB_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.AUTOCRAB_SKIP_GMAIL_WATCHER;
    } else {
      process.env.AUTOCRAB_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.AUTOCRAB_SKIP_PROVIDERS;
    } else {
      process.env.AUTOCRAB_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeEnvRefConfig() {
    const configPath = process.env.AUTOCRAB_CONFIG_PATH;
    if (!configPath) {
      throw new Error("AUTOCRAB_CONFIG_PATH is not set");
    }
    await fs.writeFile(
      configPath,
      `${JSON.stringify(
  ...
  async function writeAuthProfileEnvRefStore() {
    const stateDir = process.env.AUTOCRAB_STATE_DIR;
    if (!stateDir) {
      throw new Error("AUTOCRAB_STATE_DIR is not set");
    }
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "MISSING_AUTOCRAB_AUTH_REF" },
            },
          },
  ...
  it("fails startup when auth-profile secret refs are unresolved", async () => {
    ...
    delete process.env.MISSING_AUTOCRAB_AUTH_REF;
    try {
      await expect(withGatewayServer(async () => {})).rejects.toThrow(
        'Environment variable "MISSING_AUTOCRAB_AUTH_REF" is missing or empty.',
      );
    } finally {
