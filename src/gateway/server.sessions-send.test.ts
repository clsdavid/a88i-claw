...
const { createAutoCrabTools } = await import("../agents/autocrab-tools.js");
...
type SessionSendTool = ReturnType<typeof createAutoCrabTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createAutoCrabTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["AUTOCRAB_GATEWAY_PORT", "AUTOCRAB_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.AUTOCRAB_GATEWAY_PORT = String(gatewayPort);
  process.env.AUTOCRAB_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("AUTOCRAB_CONFIG_PATH missing in gateway test environment");
      }
