...
export function resolveHooksConfig(cfg: AutoCrabConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AutoCrabConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-autocrab-token"] === "string"
      ? req.headers["x-autocrab-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
