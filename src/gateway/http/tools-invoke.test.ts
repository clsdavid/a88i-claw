...
let lastCreateAutoCrabToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/autocrab-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateAutoCrabToolsContext?.agentTo,
          agentThreadId: lastCreateAutoCrabToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createAutoCrabTools: (ctx: Record<string, unknown>) => {
      lastCreateAutoCrabToolsContext = ctx;
      return tools;
    },
  };
});

const { handleToolsInvokeHttpRequest } = await import("./tools-invoke.js");
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateAutoCrabToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateAutoCrabToolsContext?.allowMediaInvokeCommands).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-autocrab-message-to": "channel:24514",
        "x-autocrab-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
