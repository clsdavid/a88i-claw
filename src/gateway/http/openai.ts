...
import { sendJson, setSseHeaders, writeDone } from "./common.js";
import { handleGatewayPostJsonEndpoint } from "./endpoint-helpers.js";
...
// default to 120k to be safe for gpt-4o (128k) and other models while leaving room for response
const SAFETY_CONTEXT_TOKEN_LIMIT = 120_000;
...
function resolveAgentResponseText(result: unknown): string {
  ...
  if (!Array.isArray(payloads) || payloads.length === 0) {
    return "No response from AutoCrab.";
  }
  const content = payloads
    .map((p) => (typeof p.text === "string" ? p.text : ""))
    .filter(Boolean)
    .join("\n\n");
  return content || "No response from AutoCrab.";
}
...
export async function handleOpenAiHttpRequest(
  ...
  const model = typeof payload.model === "string" ? payload.model : "autocrab";
  const user = typeof payload.user === "string" ? payload.user : undefined;

  // Truncate context if needed to avoid provider limits (e.g. gpt-4o 128k, Claude 200k)
  const messages = asMessages(payload.messages);
  const estimatedTokens = messages.reduce(
    (acc, m) => acc + JSON.stringify(m).length / TOKEN_ESTIMATE_CHARS_PER_TOKEN,
    0,
  );

  if (estimatedTokens > SAFETY_CONTEXT_TOKEN_LIMIT) {
    let currentTokens = estimatedTokens;
    let removedCount = 0;
    // Keep at least system prompt + last user message. Removing oldest non-system message until fit.
    while (currentTokens > SAFETY_CONTEXT_TOKEN_LIMIT && messages.length > 2) {
      const idx = messages.findIndex((m) => {
        const roleUnknown = (m as { role?: unknown }).role;
        const role = typeof roleUnknown === "string" ? roleUnknown.trim() : "";
        return role !== "system" && role !== "developer";
      });

      // Don't remove the very last message (likely the prompt)
      if (idx === -1 || idx >= messages.length - 1) {
        break;
      }

      const removed = messages.splice(idx, 1)[0];
      currentTokens -= JSON.stringify(removed).length / TOKEN_ESTIMATE_CHARS_PER_TOKEN;
      removedCount++;
    }

    if (removedCount > 0) {
      logWarn(
        `openai-compat: truncated ${removedCount} messages due to context overflow (${Math.round(estimatedTokens)} -> ${Math.round(currentTokens)} tokens estimated)`,
      );
    }
  }
