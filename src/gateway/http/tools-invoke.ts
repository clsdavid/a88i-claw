...
import { getBearerToken, getHeader } from "./utils.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-autocrab-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-autocrab-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-autocrab-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-autocrab-thread-id")?.trim() || undefined;
  ...
  const allTools = createAutoCrabTools({
