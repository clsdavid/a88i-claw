...
import { handleSlackHttpRequest } from "@autocrab/slack/src/http/index";
...
import { type GatewayAuthResult, type ResolvedGatewayAuth } from "../auth/index.js";
import {
  extractHookToken,
...
import { sendGatewayAuthFailure, setDefaultSecurityHeaders } from "./common.js";
import {
...
import { handleOpenAiHttpRequest } from "./openai.js";
import { handleOpenResponsesHttpRequest } from "./openresponses.js";
import { handleToolsInvokeHttpRequest } from "./tools-invoke.js";
...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.setHeader("Content-Type", "text/plain; charset=utf-8");
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-AutoCrab-Token header (query parameters are not allowed).",
      );
      return true;
    }
