...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "AUTOCRAB_SKIP_BROWSER_CONTROL_SERVER",
  "AUTOCRAB_SKIP_GMAIL_WATCHER",
  "AUTOCRAB_SKIP_CANVAS_HOST",
  "AUTOCRAB_BUNDLED_PLUGINS_DIR",
  "AUTOCRAB_SKIP_CHANNELS",
  "AUTOCRAB_SKIP_PROVIDERS",
  "AUTOCRAB_SKIP_CRON",
  "AUTOCRAB_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.AUTOCRAB_STATE_DIR = path.join(tempHome, ".autocrab");
  delete process.env.AUTOCRAB_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.AUTOCRAB_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.AUTOCRAB_SKIP_GMAIL_WATCHER = "1";
  process.env.AUTOCRAB_SKIP_CANVAS_HOST = "1";
  process.env.AUTOCRAB_SKIP_CHANNELS = "1";
  process.env.AUTOCRAB_SKIP_PROVIDERS = "1";
  process.env.AUTOCRAB_SKIP_CRON = "1";
  process.env.AUTOCRAB_TEST_MINIMAL_GATEWAY = "1";
  process.env.AUTOCRAB_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "autocrab-test-no-bundled-extensions")
    : "autocrab-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".autocrab-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".autocrab-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__autocrabTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__autocrabTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["AUTOCRAB_GATEWAY_TOKEN"]);
  const prev = process.env.AUTOCRAB_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.AUTOCRAB_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.AUTOCRAB_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
