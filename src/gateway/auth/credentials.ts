  ...
  constructor(path: string) {
    super(
      [
        ...
        "Fix: set AUTOCRAB_GATEWAY_TOKEN/AUTOCRAB_GATEWAY_PASSWORD, pass explicit --token/--password,",
...
function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.AUTOCRAB_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.AUTOCRAB_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
