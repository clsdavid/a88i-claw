...
function hasGatewayTokenCandidate(params: {
  ...
  const envToken =
    params.env.AUTOCRAB_GATEWAY_TOKEN?.trim() || params.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return true;
  }
  if (
...
function hasGatewayTokenEnvCandidate(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.AUTOCRAB_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim());
}

function hasGatewayPasswordEnvCandidate(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.AUTOCRAB_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim());
}
