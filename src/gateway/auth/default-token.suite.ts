...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.AUTOCRAB_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.AUTOCRAB_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.AUTOCRAB_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.AUTOCRAB_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            AUTOCRAB_VERSION: " ",
            AUTOCRAB_SERVICE_VERSION: "2.4.6-service",
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `autocrab-test-device-${randomUUID()}.json`),
        nonce,
      });
