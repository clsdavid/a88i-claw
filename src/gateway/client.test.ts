...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("autocrab doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with AUTOCRAB_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with AUTOCRAB_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.AUTOCRAB_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://autocrab-gateway.ai:18789",
      onConnectError,
    });
