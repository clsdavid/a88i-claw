...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: AutoCrabConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: AutoCrabConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: AutoCrabConfig) => string[];
  isConfigured?: (account: unknown, cfg: AutoCrabConfig) => boolean;
  isEnabled?: (account: unknown, cfg: AutoCrabConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "autocrab.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ AUTOCRAB_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        autocrab: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.AUTOCRAB_GATEWAY_TOKEN;
    const prevPassword = process.env.AUTOCRAB_GATEWAY_PASSWORD;
    delete process.env.AUTOCRAB_GATEWAY_TOKEN;
    delete process.env.AUTOCRAB_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.AUTOCRAB_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.AUTOCRAB_GATEWAY_PASSWORD;
      } else {
        process.env.AUTOCRAB_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/autocrab-safe-bins"];
    const cfg: AutoCrabConfig = {
      tools: {
        exec: {
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("autocrab-sbx-browser-old\nautocrab-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "autocrab-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "autocrab-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("autocrab-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("autocrab-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "autocrab-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "autocrab-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-autocrab.json");
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: AutoCrabConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: AutoCrabConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): AutoCrabConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: AutoCrabConfig = { session: { dmScope: "main" } };
  ...
  it("keeps channel security findings when SecretRef credentials are configured but unavailable", async () => {
    await withChannelSecurityStateDir(async () => {
      const sourceConfig: AutoCrabConfig = {
        channels: {
          discord: {
            enabled: true,
      ...
      const resolvedConfig: AutoCrabConfig = {
        channels: {
          discord: {
            enabled: true,
            groupPolicy: "allowlist",
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.autocrab/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: AutoCrabConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.AUTOCRAB_GATEWAY_TOKEN;
    process.env.AUTOCRAB_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: AutoCrabConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<AutoCrabConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: AutoCrabConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.autocrab",
      configPath: "/Users/test/Dropbox/.autocrab/autocrab.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: AutoCrabConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "autocrab.json"),
        execDockerRawFn: execDockerRawUnavailable,
      });
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "autocrab.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
    ...
    const cfg: AutoCrabConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@autocrab/voice-call@1.2.3",
      ...
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@autocrab/test-hooks@1.2.3",
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      path.join(pluginDir, "package.json"),
      JSON.stringify({ name: "@autocrab/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@autocrab/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        autocrab: { extensions: ["../outside.js"] },
      }),
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.AUTOCRAB_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.AUTOCRAB_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
