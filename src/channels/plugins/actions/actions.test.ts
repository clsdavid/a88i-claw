...
function telegramCfg(): AutoCrabConfig {
  return { channels: { telegram: { botToken: "tok" } } } as AutoCrabConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: AutoCrabConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as AutoCrabConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as AutoCrabConfig;
...
function createSignalAccountOverrideCfg(): AutoCrabConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as AutoCrabConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as AutoCrabConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as AutoCrabConfig,
        accountId: undefined,
        params: {
