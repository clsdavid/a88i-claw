    ...
    collectIssues: ({ account, accountId, issues }) => {
      ...
      if (!linked) {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("autocrab channels login")} (scan QR on the gateway host).`,
        });
        return;
      }
      ...
      if (running && !connected) {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("autocrab doctor")} (or restart the gateway). If it persists, relink via channels login and check logs.`,
        });
      }
    },
  });
