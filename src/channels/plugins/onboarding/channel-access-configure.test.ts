...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: AutoCrabConfig, policy: ChannelAccessPolicy) => AutoCrabConfig;
  resolveAllowlist: (params: { cfg: AutoCrabConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: AutoCrabConfig; resolved: TResolved }) => AutoCrabConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: AutoCrabConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: AutoCrabConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: AutoCrabConfig, policy: ChannelAccessPolicy): AutoCrabConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: AutoCrabConfig, policy: ChannelAccessPolicy): AutoCrabConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: AutoCrabConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: AutoCrabConfig; resolved: string[] }) => {
      calls.push("apply");
