...
import { discoverAutoCrabPlugins } from "../../plugins/discovery.js";
import type { AutoCrabPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, AutoCrabPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["AUTOCRAB_PLUGIN_CATALOG_PATHS", "AUTOCRAB_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<AutoCrabPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: AutoCrabPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: AutoCrabPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverAutoCrabPlugins({ workspaceDir: options.workspaceDir });
