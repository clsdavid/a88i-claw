...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".autocrab");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.AUTOCRAB_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$AUTOCRAB_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via AUTOCRAB_STATE_DIR.
