...
async function runGatewayHealthCheck(params: {
  ...
  const token =
    process.env.AUTOCRAB_GATEWAY_TOKEN ?? process.env.CLAWDBOT_GATEWAY_TOKEN ?? configuredToken;
  const password =
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from autocrab.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "AutoCrab update wizard" : "AutoCrab configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: AutoCrabConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("autocrab doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
