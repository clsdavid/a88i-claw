...
export function requireAutoCrabAgentDir(): string {
  const agentDir = process.env.AUTOCRAB_AGENT_DIR;
  if (!agentDir) {
    throw new Error("AUTOCRAB_AGENT_DIR not set");
  }
  return agentDir;
}
