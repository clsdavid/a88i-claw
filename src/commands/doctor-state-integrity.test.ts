...
type EnvSnapshot = {
  HOME?: string;
  AUTOCRAB_HOME?: string;
  AUTOCRAB_STATE_DIR?: string;
  AUTOCRAB_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    AUTOCRAB_HOME: process.env.AUTOCRAB_HOME,
    AUTOCRAB_STATE_DIR: process.env.AUTOCRAB_STATE_DIR,
    AUTOCRAB_OAUTH_DIR: process.env.AUTOCRAB_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: AutoCrabConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: AutoCrabConfig) {
...
async function runStateIntegrityText(cfg: AutoCrabConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "autocrab-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.AUTOCRAB_HOME = tempHome;
    process.env.AUTOCRAB_STATE_DIR = path.join(tempHome, ".autocrab");
    delete process.env.AUTOCRAB_OAUTH_DIR;
    fs.mkdirSync(process.env.AUTOCRAB_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when AUTOCRAB_OAUTH_DIR is explicitly configured", async () => {
    process.env.AUTOCRAB_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: AutoCrabConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints autocrab-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: AutoCrabConfig = {};
    ...
    expect(text).toMatch(/autocrab sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/autocrab sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /autocrab sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
