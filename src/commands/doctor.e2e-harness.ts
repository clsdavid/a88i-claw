...
export const resolveAutoCrabPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadAutoCrabPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveAutoCrabPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.AUTOCRAB_STATE_DIR;
  originalUpdateInProgress = process.env.AUTOCRAB_UPDATE_IN_PROGRESS;
  process.env.AUTOCRAB_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "autocrab-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.AUTOCRAB_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.AUTOCRAB_UPDATE_IN_PROGRESS;
  } else {
    process.env.AUTOCRAB_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
