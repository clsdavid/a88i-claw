...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under AUTOCRAB_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("autocrab-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-oauth-sync-"));
    ...
    process.env.AUTOCRAB_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-oauth-nosync-"));
    process.env.AUTOCRAB_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside AUTOCRAB_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-oauth-external-"));
    process.env.AUTOCRAB_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* AUTOCRAB_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to AUTOCRAB_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("autocrab-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
