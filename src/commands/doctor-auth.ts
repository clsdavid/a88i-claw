...
function pruneAuthProfiles(
  cfg: AutoCrabConfig,
  profileIds: Set<string>,
): { next: AutoCrabConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("autocrab models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("autocrab models auth setup-token")} or ${formatCliCommand(
      "autocrab configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "autocrab models auth login --provider openai-codex",
    )} or ${formatCliCommand("autocrab configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("autocrab configure")}\` or \`${formatCliCommand("autocrab onboard")}\`.`;
}
