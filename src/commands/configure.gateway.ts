...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "AUTOCRAB_GATEWAY_TOKEN",
          placeholder: "AUTOCRAB_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like AUTOCRAB_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. AutoCrab will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: AutoCrab trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.autocrab.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
