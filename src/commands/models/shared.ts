...
export async function loadValidConfigOrThrow(): Promise<AutoCrabConfig> {
...
export async function updateConfig(
  mutator: (cfg: AutoCrabConfig) => AutoCrabConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: AutoCrabConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AutoCrabConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("autocrab agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
