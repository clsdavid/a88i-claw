...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAutoCrabAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("autocrab models auth setup-token")}\` or \`${formatCliCommand("autocrab configure")}\`.`
          : `Run \`${formatCliCommand("autocrab configure")}\` or set an API key env var.`;
