...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAutoCrabAgentDir: vi.fn().mockReturnValue("/tmp/autocrab-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/autocrab-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/autocrab-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAutoCrabAgentDir: mocks.resolveAutoCrabAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/autocrab-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAutoCrabAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/autocrab-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/autocrab-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/autocrab-agent-custom");
