...
function resolveAgentName(cfg: AutoCrabConfig, agentId: string) {
...
function resolveAgentModel(cfg: AutoCrabConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AutoCrabConfig): AgentSummary[] {
