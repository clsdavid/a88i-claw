...
type PromptDefaultModelResult = { model?: string; config?: AutoCrabConfig };
...
function resolveConfiguredModelRaw(cfg: AutoCrabConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: AutoCrabConfig): string[] {
...
export function applyPrimaryModel(cfg: AutoCrabConfig, model: string): AutoCrabConfig {
...
export function applyModelAllowlist(cfg: AutoCrabConfig, models: string[]): AutoCrabConfig {
  const defaults = cfg.agents?.defaults;
