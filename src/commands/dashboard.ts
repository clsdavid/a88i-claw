...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv): string | undefined {
  const primary = env.AUTOCRAB_GATEWAY_TOKEN?.trim();
  if (primary) {
    return primary;
  }
...
export async function dashboardCommand(
  ...
  if (resolvedToken.unresolvedRefReason) {
    ...
    runtime.log(
      "Set AUTOCRAB_GATEWAY_TOKEN in this shell or resolve your secret provider, then rerun `autocrab dashboard`.",
    );
  }
  ...
  if (opened) {
    runtime.log("Opened in your browser. Keep that tab to control AutoCrab.");
  } else if (hint) {
    runtime.log(hint);
  }
}
