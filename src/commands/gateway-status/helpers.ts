...
import type { AutoCrabConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: AutoCrabConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const token = env.AUTOCRAB_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  return token || undefined;
}

function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const password = env.AUTOCRAB_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim();
...
export function buildNetworkHints(cfg: AutoCrabConfig) {
