...
beforeAll(() => {
  envSnapshot = captureEnv(["AUTOCRAB_PROFILE"]);
  process.env.AUTOCRAB_PROFILE = "isolated";
});
...
vi.mock("../infra/autocrab-root.js", () => ({
  resolveAutoCrabPackageRoot: vi.fn().mockResolvedValue("/tmp/autocrab"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/autocrab/pnpm-lock.yaml",
      markerPath: "/tmp/autocrab/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.autocrab.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.autocrab.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("autocrab status --all") ||
          line.includes("autocrab --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("AUTOCRAB_GATEWAY_TOKEN", "abcd1234", async () => {
