...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: AutoCrabConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: AutoCrabConfig) => { changed: boolean; next: AutoCrabConfig };
  defaultModel: string;
  overrideConfig: AutoCrabConfig;
  alreadyDefaultConfig: AutoCrabConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: AutoCrabConfig) => config,
      applyDefaultConfig: (config: AutoCrabConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: AutoCrabConfig = { agents: { defaults: {} } };
