...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "AutoCrab", ".autocrab");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/autocrab-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".autocrab");
  ...
  it("anchors cloud detection to OS homedir when AUTOCRAB_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".autocrab");
    const originalAutoCrabHome = process.env.AUTOCRAB_HOME;
    process.env.AUTOCRAB_HOME = "/tmp/autocrab-home-override";
    ...
    try {
      ...
      if (originalAutoCrabHome === undefined) {
        delete process.env.AUTOCRAB_HOME;
      } else {
        process.env.AUTOCRAB_HOME = originalAutoCrabHome;
      }
    }
  });
