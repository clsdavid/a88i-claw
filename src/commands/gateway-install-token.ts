...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("autocrab config set gateway.auth.mode token")} or ${formatCliCommand("autocrab config set gateway.auth.mode password")}.`,
  ].join(" ");
}
...
export async function resolveGatewayInstallToken(
  ...
  const envToken =
    options.env.AUTOCRAB_GATEWAY_TOKEN?.trim() || options.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
