...
import { noteLegacyStateDirUsage } from "./doctor-legacy-config.js";
...
function resolveMode(cfg: AutoCrabConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("AutoCrab doctor");

  const root = await resolveAutoCrabPackageRoot({
  ...
  let cfg: AutoCrabConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("autocrab configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("autocrab config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("autocrab setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("autocrab config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("autocrab config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("autocrab doctor --fix")}" to apply changes.`);
  }
