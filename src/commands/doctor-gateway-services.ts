...
export async function maybeRepairGatewayServiceConfig(
  ...
  const serviceToken = command.environment?.AUTOCRAB_GATEWAY_TOKEN?.trim();
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      code: SERVICE_AUDIT_CODES.gatewayTokenMismatch,
      message:
        "Gateway service AUTOCRAB_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing AutoCrab gateway next.");
      }
    }
  }
