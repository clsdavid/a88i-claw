...
import type { AutoCrabConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: AutoCrabConfig): string[] {
...
export async function noteSecurityWarnings(cfg: AutoCrabConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("autocrab security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.autocrab/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("autocrab approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.autocrab.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("autocrab configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("autocrab config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("autocrab doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("autocrab config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('autocrab config set session.dmScope "per-channel-peer"') +
