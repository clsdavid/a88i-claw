...
function prependInternalEventContext(
  ...
  if (body.includes("AutoCrab runtime context (internal):")) {
    return body;
  }
...
async function agentCommandInternal(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("autocrab agents list")}" to see configured agents.`,
      );
    }
  }
