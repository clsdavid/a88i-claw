...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    pluginRegistry.list = [];
    prevToken = process.env.AUTOCRAB_GATEWAY_TOKEN;
    prevPassword = process.env.AUTOCRAB_GATEWAY_PASSWORD;
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as AutoCrabConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.AUTOCRAB_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as AutoCrabConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats SecretRef token config as authenticated for exposure warning level", async () => {
    const cfg = {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: { source: "env", provider: "default", id: "AUTOCRAB_GATEWAY_TOKEN" },
        },
      },
    } as AutoCrabConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as AutoCrabConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as AutoCrabConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("autocrab approvals get --gateway");
  });
