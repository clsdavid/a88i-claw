...
const SEARCH_TOOL = "https://docs.autocrab.ai/mcp.SearchAutoCrab";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.autocrab.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('autocrab docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.autocrab.ai/");
      runtime.log(`Search: ${formatCliCommand('autocrab docs "your query"')}`);
    }
    return;
  }
