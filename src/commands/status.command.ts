...
export async function statusCommand(
  ...
  runtime.log(theme.heading("AutoCrab status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`autocrab devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("autocrab devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("autocrab devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("autocrab security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("autocrab security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.autocrab.ai/faq");
  runtime.log("Troubleshooting: https://docs.autocrab.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("autocrab status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("autocrab logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("autocrab status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("autocrab gateway probe")}`);
  }
}
