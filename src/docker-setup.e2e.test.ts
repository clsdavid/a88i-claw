...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "autocrab-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    AUTOCRAB_GATEWAY_TOKEN: "test-token",
    AUTOCRAB_CONFIG_DIR: join(sandbox.rootDir, "config"),
    AUTOCRAB_WORKSPACE_DIR: join(sandbox.rootDir, "autocrab"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      AUTOCRAB_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      AUTOCRAB_EXTRA_MOUNTS: undefined,
      AUTOCRAB_HOME_VOLUME: "autocrab-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("AUTOCRAB_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("AUTOCRAB_EXTRA_MOUNTS=");
    expect(envFile).toContain("AUTOCRAB_HOME_VOLUME=autocrab-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("autocrab-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("autocrab-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg AUTOCRAB_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain("run --rm autocrab-cli onboard --mode local --no-install-daemon");
    expect(log).toContain("run --rm autocrab-cli config set gateway.mode local");
    expect(log).toContain("run --rm autocrab-cli config set gateway.bind lan");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      AUTOCRAB_CONFIG_DIR: configDir,
      AUTOCRAB_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when AUTOCRAB_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    await writeFile(
      join(configDir, "autocrab.json"),
      JSON.stringify({ gateway: { auth: { mode: "token", token: "config-token-123" } } }),
    );

    const result = runDockerSetup(activeSandbox, {
      AUTOCRAB_GATEWAY_TOKEN: undefined,
      AUTOCRAB_CONFIG_DIR: configDir,
      AUTOCRAB_WORKSPACE_DIR: workspaceDir,
    });
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("AUTOCRAB_GATEWAY_TOKEN=config-token-123");
  });

  it("treats AUTOCRAB_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("AUTOCRAB_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg AUTOCRAB_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg AUTOCRAB_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      AUTOCRAB_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker autocrab-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        AUTOCRAB_SANDBOX: "1",
        AUTOCRAB_DOCKER_SOCKET: socketPath,
      ...
      const gatewayStarts = log
        ...
        .filter(
          (line) =>
            ...
            line.includes(" up -d") &&
            line.includes("autocrab-gateway"),
        );
      expect(gatewayStarts).toHaveLength(2);
      expect(log).toContain(
        "run --rm --no-deps autocrab-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate autocrab-gateway"));
  ...
  it("rejects injected multiline AUTOCRAB_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      AUTOCRAB_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("AUTOCRAB_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid AUTOCRAB_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      AUTOCRAB_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid AUTOCRAB_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      AUTOCRAB_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("AUTOCRAB_HOME_VOLUME must match");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:autocrab-gateway"');
