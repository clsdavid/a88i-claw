...
function ensureNodePathEnv(): string {
  ensureAutoCrabCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
export async function resolveNodeHostGatewayCredentials(params: {
  ...
  const token =
    normalizeSecretInputString(env.AUTOCRAB_GATEWAY_TOKEN) ??
  ...
  const shouldResolveConfiguredPassword =
    !normalizeSecretInputString(env.AUTOCRAB_GATEWAY_PASSWORD) &&
    !tokenCanWin &&
