...
function resolveAuth(cfg: AutoCrabConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.AUTOCRAB_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayTokenSecretRef(
  ...
  const hasTokenEnvCandidate = Boolean(
    env.AUTOCRAB_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim(),
  );
  ...
  if (mode !== "token") {
    const hasPasswordEnvCandidate = Boolean(
      env.AUTOCRAB_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim(),
    );
...
async function resolveGatewayPasswordSecretRef(
  ...
  if (mode !== "password") {
    const hasTokenCandidate =
      Boolean(env.AUTOCRAB_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim()) ||
