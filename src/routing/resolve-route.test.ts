...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: AutoCrabConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: AutoCrabConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/autocrab-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<AutoCrabConfig["bindings"]>[number];
