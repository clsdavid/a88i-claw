...
describe("web processMessage inbound contract", () => {
  beforeEach(async () => {
    ...
    sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "autocrab-process-message-"));
  ...
  it("does not force an [autocrab] response prefix in self-chats when identity is unset", async () => {
    capturedDispatchParams = undefined;
