...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/autocrab/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/autocrab");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/autocrab/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__AUTOCRAB_CONTROL_UI_BASE_PATH__ = "/autocrab";
    const app = mountApp("/autocrab/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/autocrab");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/autocrab/sessions");
  });
