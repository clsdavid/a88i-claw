...
# Build and bundle AutoCrab into a minimal .app we can open.
# Outputs to dist/AutoCrab.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/AutoCrab.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="AutoCrab"
BUNDLE_ID="${BUNDLE_ID:-ai.autocrab.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/autocrab/autocrab/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/AutoCrab/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AutoCrabBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AutoCrabGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/AutoCrab"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/AutoCrab"
fi
chmod +x "$APP_ROOT/Contents/MacOS/AutoCrab"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/AutoCrab" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/AutoCrab/Resources/AutoCrab.icns" "$APP_ROOT/Contents/Resources/AutoCrab.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/AutoCrab/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AutoCrabKit resources"
AUTOCRABKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AutoCrabKit_AutoCrabKit.bundle"
if [ -d "$AUTOCRABKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AutoCrabKit_AutoCrabKit.bundle"
  cp -R "$AUTOCRABKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AutoCrabKit_AutoCrabKit.bundle"
else
  echo "WARN: AutoCrabKit resource bundle not found at $AUTOCRABKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running AutoCrab"
killall -q AutoCrab 2>/dev/null || true
