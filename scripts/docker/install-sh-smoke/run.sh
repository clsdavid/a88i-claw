...
INSTALL_URL="${AUTOCRAB_INSTALL_URL:-https://autocrab.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${AUTOCRAB_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${AUTOCRAB_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="autocrab"
PACKAGE_NAME="${AUTOCRAB_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (AUTOCRAB_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${AUTOCRAB_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${AUTOCRAB_INSTALL_LATEST_OUT:-}"
fi
