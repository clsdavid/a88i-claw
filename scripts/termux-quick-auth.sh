...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/autocrab/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "AutoCrab Re-Auth" -c "After getting key, run: ssh $SERVER '~/autocrab/scripts/mobile-reauth.sh'" --id autocrab-auth
        ;;
    *)
