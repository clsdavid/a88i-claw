...
#
# Suggested cron: */30 * * * * /home/admin/autocrab/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send AutoCrab notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., autocrab-alerts)
...
STATE_FILE="$HOME/.autocrab/auth-monitor-state"
...
send_notification() {
    ...
    # Send via AutoCrab if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use autocrab
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via AutoCrab to $NOTIFY_PHONE..."
            autocrab send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! AutoCrab is down. Run: ssh l36 '~/autocrab/scripts/mobile-reauth.sh'" "urgent"
    exit 1
