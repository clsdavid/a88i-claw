#!/usr/bin/env bash
# ClawDock - Docker helpers for AutoCrab
# Inspired by Simon Willison's "Running AutoCrab in Docker"
# https://til.simonwillison.net/llms/autocrab-docker
#
# Installation:
...
# Common paths to check for AutoCrab
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/autocrab"
  "${HOME}/workspace/autocrab"
  "${HOME}/projects/autocrab"
  "${HOME}/dev/autocrab"
  "${HOME}/code/autocrab"
  "${HOME}/src/autocrab"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found AutoCrab at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/autocrab"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ AutoCrab not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^AUTOCRAB_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d autocrab-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart autocrab-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f autocrab-gateway
}
...
clawdock-workspace() {
  cd ~/.autocrab/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec autocrab-gateway \
    bash -c 'echo "alias autocrab=\"./autocrab.mjs\"" > /tmp/.bashrc_autocrab && bash --rcfile /tmp/.bashrc_autocrab'
}
...
clawdock-cli() {
  _clawdock_compose run --rm autocrab-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build autocrab-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "AUTOCRAB_GATEWAY_TOKEN=$token" autocrab-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" autocrab-gateway \
    bash -c './autocrab.mjs config set gateway.remote.token "$TOKEN" && ./autocrab.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec autocrab-gateway \
    bash -c "./autocrab.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart autocrab-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm autocrab-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec autocrab-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'autocrab config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for AutoCrab${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (autocrab alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to autocrab project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.autocrab)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'autocrab channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'autocrab status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.autocrab.ai${_CLR_RESET}"
  echo ""
}
