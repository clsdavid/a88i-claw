...
BASE_IMAGE="${BASE_IMAGE:-autocrab-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-autocrab-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=autocrab.sandbox=1)
NOTE
