...
# AutoCrab Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://autocrab.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one AutoCrab."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
GUM_VERSION="${AUTOCRAB_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 AutoCrab Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://autocrab.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$AUTOCRAB_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.autocrab.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_autocrab_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.autocrab-* "$npm_root"/autocrab 2>/dev/null || true
}

extract_autocrab_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_autocrab_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/autocrab" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/autocrab"|"/usr/local/bin/autocrab")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/autocrab/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale autocrab symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing autocrab binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing AutoCrab package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_autocrab_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "AutoCrab npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*autocrab" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_autocrab_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_autocrab_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "AutoCrab npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an autocrab binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "AUTOCRAB_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "AUTOCRAB_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "AUTOCRAB_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "AUTOCRAB_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "AUTOCRAB_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "AUTOCRAB_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "AUTOCRAB_BETA" "CLAWDBOT_BETA"
map_legacy_env "AUTOCRAB_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "AUTOCRAB_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "AUTOCRAB_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "AUTOCRAB_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "AUTOCRAB_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "AUTOCRAB_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${AUTOCRAB_TAGLINE_INDEX:-}" ]]; then
        if [[ "${AUTOCRAB_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((AUTOCRAB_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${AUTOCRAB_NO_ONBOARD:-0}
NO_PROMPT=${AUTOCRAB_NO_PROMPT:-0}
DRY_RUN=${AUTOCRAB_DRY_RUN:-0}
INSTALL_METHOD=${AUTOCRAB_INSTALL_METHOD:-}
AUTOCRAB_VERSION=${AUTOCRAB_VERSION:-latest}
USE_BETA=${AUTOCRAB_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/autocrab"
GIT_DIR=${AUTOCRAB_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${AUTOCRAB_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${AUTOCRAB_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${AUTOCRAB_VERBOSE:-0}"
AUTOCRAB_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
AutoCrab installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://autocrab.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/autocrab)
...
Environment variables:
  AUTOCRAB_INSTALL_METHOD=git|npm
  AUTOCRAB_VERSION=latest|next|<semver>
  AUTOCRAB_BETA=0|1
  AUTOCRAB_GIT_DIR=...
  AUTOCRAB_GIT_UPDATE=0|1
  AUTOCRAB_NO_PROMPT=1
  AUTOCRAB_DRY_RUN=1
  AUTOCRAB_NO_ONBOARD=1
  AUTOCRAB_VERBOSE=1
  AUTOCRAB_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://autocrab.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://autocrab.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://autocrab.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected AutoCrab checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a AutoCrab source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_autocrab_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"autocrab"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://autocrab.ai/install.sh | bash"
}
...
ensure_autocrab_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/autocrab" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/autocrab" ]]; then
        ln -sf "$npm_root/autocrab/dist/entry.js" "${npm_bin}/autocrab"
        ui_info "Created autocrab bin link at ${npm_bin}/autocrab"
    fi
    return 0
}

# Check for existing AutoCrab installation
check_existing_autocrab() {
    if [[ -n "$(type -P autocrab 2>/dev/null || true)" ]]; then
        ui_info "Existing AutoCrab installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make autocrab show as \"command not found\" in new terminals."
...
warn_autocrab_not_found() {
    ui_warn "Installed, but autocrab is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t autocrab 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named autocrab; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_autocrab_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P autocrab 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/autocrab" ]]; then
        echo "${npm_bin}/autocrab"
        return 0
    fi
...
install_autocrab_from_git() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ...
        ui_info "Installing AutoCrab from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning AutoCrab" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building AutoCrab" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/autocrab" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/autocrab"
    ui_success "AutoCrab wrapper installed to \$HOME/.local/bin/autocrab"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view autocrab dist-tags.beta 2>/dev/null || true)"
...
install_autocrab() {
    local package_name="autocrab"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            AUTOCRAB_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="autocrab"
        else
            AUTOCRAB_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${AUTOCRAB_VERSION}" ]]; then
        AUTOCRAB_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${AUTOCRAB_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing AutoCrab v${resolved_version}"
    else
        ui_info "Installing AutoCrab (${AUTOCRAB_VERSION})"
    fi
    local install_spec=""
    if [[ "${AUTOCRAB_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${AUTOCRAB_VERSION}"
    fi

    if ! install_autocrab_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_autocrab_paths
        install_autocrab_npm "${install_spec}"
    fi

    if [[ "${AUTOCRAB_VERSION}" == "latest" && "${package_name}" == "autocrab" ]]; then
        if ! resolve_autocrab_bin &> /dev/null; then
            ui_warn "npm install autocrab@latest failed; retrying autocrab@next"
            cleanup_npm_autocrab_paths
            install_autocrab_npm "autocrab@next"
        fi
    fi

    ensure_autocrab_bin_link || true

    ui_success "AutoCrab installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${AUTOCRAB_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_autocrab_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (autocrab not on PATH yet)"
        warn_autocrab_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${AUTOCRAB_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.autocrab/workspace-${profile}"
    else
        echo "${HOME}/.autocrab/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${AUTOCRAB_CONFIG_PATH:-$HOME/.autocrab/autocrab.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run autocrab onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but autocrab not on PATH; skipping onboarding"
        warn_autocrab_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run autocrab onboard to retry"
        return
    }
}

resolve_autocrab_version() {
    local version=""
    local claw="${AUTOCRAB_BIN:-}"
    if [[ -z "$claw" ]] && command -v autocrab &> /dev/null; then
        claw="$(command -v autocrab)"
    fi
    if [[ -n "$claw" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/autocrab/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/autocrab/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_autocrab_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found AutoCrab checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set AUTOCRAB_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_autocrab; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing AutoCrab"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g autocrab &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g autocrab 2>/dev/null || true
        ...
        install_autocrab_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/autocrab" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/autocrab"
    ...
    AUTOCRAB_BIN="$(resolve_autocrab_bin || true)"
    ...
    installed_version=$(resolve_autocrab_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 AutoCrab installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 AutoCrab installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/autocrab"
        ui_kv "Update command" "autocrab update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://autocrab.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running autocrab doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                AUTOCRAB_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                AUTOCRAB_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                AUTOCRAB_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run autocrab doctor and autocrab plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run autocrab onboard later"
        else
            local config_path="${AUTOCRAB_CONFIG_PATH:-$HOME/.autocrab/autocrab.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (autocrab not on PATH yet)"
                    warn_autocrab_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run autocrab onboard to finish setup"
            return 0
        fi
    fi

    if command -v autocrab &> /dev/null; then
        local claw="${AUTOCRAB_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_autocrab_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (autocrab daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if AUTOCRAB_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: autocrab daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${AUTOCRAB_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
