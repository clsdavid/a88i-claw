...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "AutoCrab")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "AutoCrab")
...
DMG_TEMP="$(mktemp -d /tmp/autocrab-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/AutoCrab/Resources/AutoCrab.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/autocrab-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/autocrab-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/autocrab-dmg-limits.txt
