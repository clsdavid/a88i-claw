# AutoCrab Installer for Windows (PowerShell)
# Usage: iwr -useb https://autocrab.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://autocrab.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\autocrab",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 AutoCrab Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one AutoCrab.$NC" -Level info
    Write-Host ""
}
...
function Install-AutoCrabNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing AutoCrab (autocrab@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g autocrab@$Version --no-fund --no-audit 2>&1
        Write-Host "AutoCrab installed" -Level success
        return $true
    } catch {
...
function Install-AutoCrabGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing AutoCrab from git..." -Level info
    ...
    @"
@echo off
node "%~dp0..\autocrab\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\autocrab.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install AutoCrab from git to $GitDir" -Level info
        } else {
            Install-AutoCrabGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install AutoCrab via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-AutoCrabNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'autocrab onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 AutoCrab installed successfully!" -Level success
}
