...
IMAGE_NAME="autocrab-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="autocrab-net-e2e-$$"
GW_NAME="autocrab-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "AUTOCRAB_GATEWAY_TOKEN=$TOKEN" \
  -e "AUTOCRAB_SKIP_CHANNELS=1" \
  -e "AUTOCRAB_SKIP_GMAIL_WATCHER=1" \
  -e "AUTOCRAB_SKIP_CRON=1" \
  -e "AUTOCRAB_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
