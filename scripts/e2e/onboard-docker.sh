...
IMAGE_NAME="autocrab-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  set -euo pipefail
	  ...
	  if [ -f dist/index.mjs ]; then
	    AUTOCRAB_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    AUTOCRAB_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export AUTOCRAB_ENTRY

  # Provide a minimal trash shim to avoid noisy "missing trash" logs in containers.
  export PATH="/tmp/autocrab-bin:$PATH"
  mkdir -p /tmp/autocrab-bin
  cat > /tmp/autocrab-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/autocrab-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$AUTOCRAB_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_autocrab_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/autocrab-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/autocrab-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$AUTOCRAB_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/autocrab-e2e-$1.XXXXXX"
  }

  set_isolated_autocrab_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export AUTOCRAB_HOME="$home_dir"
    export AUTOCRAB_STATE_DIR="$home_dir/.autocrab"
    export AUTOCRAB_CONFIG_PATH="$AUTOCRAB_STATE_DIR/autocrab.json"
    mkdir -p "$AUTOCRAB_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$AUTOCRAB_STATE_DIR/workspace"
    config_path="$AUTOCRAB_CONFIG_PATH"
    sessions_dir="$AUTOCRAB_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_autocrab_env "$home_dir"
	    ...
	    node "$AUTOCRAB_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$AUTOCRAB_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$AUTOCRAB_ENTRY\" configure --section skills" send_skills_flow
