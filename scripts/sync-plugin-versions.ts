...
function stripWorkspaceAutocrabDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.autocrab !== "workspace:*") {
    return false;
  }
  delete devDeps.autocrab;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceAutocrabDevDependency(pkg);
