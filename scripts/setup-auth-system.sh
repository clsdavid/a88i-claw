#!/bin/bash
# Setup AutoCrab Auth Management System
...
echo "=== AutoCrab Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'autocrab-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  AutoCrab message: Send warning via AutoCrab itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/autocrab-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/autocrab-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/autocrab-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now autocrab-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status autocrab-auth-monitor.timer"
echo ""
echo "Quick commands:"
