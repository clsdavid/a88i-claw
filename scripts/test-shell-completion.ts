 ...
 * `autocrab update`. Use it to verify the completion installation flow
...
\`autocrab update\`, \`autocrab doctor\`, and \`autocrab onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# AutoCrab Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`autocrab completion --install\` later to enable.`));
    return;
  }
