...
const shouldBuild = (deps) => {
  if (deps.env.AUTOCRAB_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.AUTOCRAB_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[autocrab] ${message}\n`);
};

const runAutoCrab = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["autocrab.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
