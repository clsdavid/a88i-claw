...
If you believe you've found a security issue in AutoCrab, please report it privately.
...
For full reporting instructions see our [Trust page](https://trust.autocrab.ai).
...
- Tested version details (AutoCrab version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to AutoCrab's documented trust boundaries.
- For exposed-secret reports: proof the credential is AutoCrab-owned (or grants access to AutoCrab-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at AutoCrab. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

AutoCrab is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
AutoCrab does **not** model one gateway as a multi-tenant, adversarial user boundary.

- Authenticated Gateway callers are treated as trusted operators for that gateway instance.
- Session identifiers (`sessionKey`, session IDs, labels) are routing controls, not per-user authorization boundaries.
- If one operator can view data from another operator on the same gateway, that is expected in this trust model.
- AutoCrab can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need AutoCrab, use one VPS (or host/OS user boundary) per user.
...
- This is expected in AutoCrab's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of AutoCrab's trusted computing base for a gateway.
...
- Using AutoCrab in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.autocrab`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not AutoCrab-owned and not granting access to AutoCrab-operated infrastructure/services) without demonstrated AutoCrab impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

AutoCrab security guidance assumes:

- The host where AutoCrab runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.autocrab` state/config (including `autocrab.json`) is effectively a trusted operator.
...
AutoCrab's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
AutoCrab separates routing from execution, but both remain inside the same operator trust boundary:
...
- Plugins can execute with the same OS privileges as the AutoCrab process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

AutoCrab uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/autocrab` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/autocrab` (or `autocrab-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the AutoCrab-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use AutoCrab temp helpers (`resolvePreferredAutoCrabTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-autocrab-dir.ts`
...
For threat model + hardening guidance (including `autocrab security audit --deep` and `--fix`), see:

- `https://docs.autocrab.ai/gateway/security`
...
AutoCrab's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `autocrab gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - AutoCrab keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `autocrab security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__autocrab__/canvas/`, `/__autocrab__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__autocrab__/canvas/`, `/__autocrab__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

AutoCrab requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running AutoCrab in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v autocrab-data:/app/data \
  autocrab/autocrab:latest
```
