...
This document describes how the AutoCrab ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`autocrab acp` exposes an ACP agent over stdio and forwards prompts to a running
AutoCrab Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `autocrab acp` and speaks ACP messages over stdio.
...
`autocrab acp` resolves the Gateway URL and auth from CLI flags or config:
